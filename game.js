...
function makeOpponents(playerVehicle, age, director) {
  ...
  const scenario = activeScenario();
  const routeLength = raceLength(selectedRace);
  const waveGap = Math.max(1350, Math.min(2600, routeLength / 4.8));
  const grid = [
    { gap: Math.min(980, waveGap * 0.62), lane: -1.95, bias: 0.58, speed: 30 },
    { gap: waveGap * 1.22, lane: 1.95, bias: 0.66, speed: 34 },
    { gap: -waveGap * 0.82, lane: 1.18, bias: 0.76, speed: 36 },
    { gap: waveGap * 2.18, lane: -0.74, bias: 0.72, speed: 40 },
    { gap: -waveGap * 1.86, lane: -1.56, bias: 0.86, speed: 41 },
    { gap: waveGap * 3.28, lane: 0.74, bias: 0.78, speed: 44 },
    { gap: -waveGap * 2.72, lane: 1.95, bias: 0.9, speed: 42 },
    { gap: waveGap * 4.34, lane: -1.18, bias: 0.84, speed: 48 }
  ];
  const allyGrid = [
  ...
  return roster.map((entry, index) => {
    ...
    const laneJitter = (Math.random() - 0.5) * 0.16;
    const startGap = slot.gap + (Math.random() - 0.5) * 42;
    return {
      name,
      vehicleId: vehicle.id,
      lane: Math.max(-2.08, Math.min(2.08, slot.lane + laneJitter)),
      homeLane: Math.max(-2.08, Math.min(2.08, slot.lane + laneJitter)),
      distance: startGap,
...
function enforceOpponentSpacing(length, dt) {
  ...
  const phoneMode = phoneGraphicsActive();
  const minGap = phoneMode ? 1680 : 1380;
  ...
  active.forEach((opponent, index) => {
    ...
    if (playerGap > -520 && playerGap < 2500 && !opponent.wrecked) {
      const nearPlayerLane = Math.abs(opponent.lane - raceState.lane) < 1.02 && playerGap > -140 && playerGap < 620;
      const laneConflict = active.some((other) => {
        if (other === opponent || other.finished) return false;
        const otherGap = other.distance - opponent.distance;
        return Math.abs(otherGap) < (phoneMode ? 560 : 430) && Math.abs(other.lane - opponent.lane) < 0.82;
      });
      const openSlot = laneSlots.find((slot) => Math.abs(slot - raceState.lane) > 1.05 && active.every((other) => {
        ...
        return Math.abs(otherGap - playerGap) > (phoneMode ? 560 : 430) || Math.abs(other.lane - slot) > 0.82;
      }));
...
function tick(dt) {
  ...
  const routeProgress = raceLength() ? Math.max(0, Math.min(1, raceState.distance / raceLength())) : 0;
  const roadCondition = routeWeatherCondition(selectedRace.place, routeProgress, raceState.elapsed || 0);
  raceState.roadCondition = roadCondition;
  const roadGrip = weatherRoadGrip(roadCondition.surface);
  const roadSpeed = weatherRoadSpeed(roadCondition.surface);
  ...
  const maxSpeed = (245 + upgrades.engine * 26) * age.speed * vehicle.speed * surfaceBoost * performanceHealth * roadSpeed;
  ...
  const throttleAccel = (42 + upgrades.engine * 4.8) * age.speed * vehicle.speed * surfaceBoost * performanceHealth * roadSpeed;
  ...
  const steerTarget = steerInput * (0.92 + upgrades.tires * 0.045) * handlingHealth * roadGrip * (raceState.speed < -1 ? -0.62 : 1);
  raceState.steerAngle += (steerTarget - raceState.steerAngle) * Math.min(1, dt * (5.4 + vehicle.handling * 2.2) * handlingHealth);
  const lateralAccel = raceState.steerAngle * speedGrip * (4.2 + vehicle.handling * 1.35 + upgrades.tires * 0.28) * handlingHealth * roadGrip;
  raceState.lateralVelocity += lateralAccel * dt;
  const grip = (2.15 + vehicle.handling * 1.25 + upgrades.tires * 0.32 + (brakeInput ? 0.75 : 0)) * handlingHealth * roadGrip;
  raceState.lateralVelocity *= Math.max(0, 1 - dt * grip);
  if (Math.abs(steerInput) > 0.05 && Math.abs(raceState.speed) < 46) {
    raceState.lateralVelocity += steerInput * dt * (2.1 + vehicle.handling * 0.48) * handlingHealth * roadGrip;
  }
  ...
  raceState.slip = Math.min(1, Math.abs(raceState.lateralVelocity) * 0.42 + Math.abs(raceState.steerAngle) * speedGrip * 0.18 + raceState.brakeHeat * 0.22 + offRoad * 0.5 + (1 - roadGrip) * 0.34);
  ...
  const roadShake = roadCondition.hazard === "earthquake" ? Math.sin((raceState.elapsed || 0) * 9.5) * 0.08 : 0;
  const turnTarget = roadTurnAt(raceState.distance) + roadShake;
  ...
  if (roadCondition.hazard === "earthquake") raceState.cameraShake = Math.max(raceState.cameraShake, 1.4 + Math.abs(roadShake) * 18);
...
function routeStageInfo(place = "city", progress = 0) {
  ...
  const stages = {
    ...
    vegas: [["Neon Boulevard", "hotelBlock"], ["Casino Tunnel", "tunnel"], ["Desert Beltway", "neon"], ["Strip Finish", "checkpoint"]],
    dubai: [["Marina Towers", "glassTower"], ["Glass Tunnel", "tunnel"], ["Palm Causeway", "marinaYacht"], ["Marina Finish", "checkpoint"]],
    venice: [["Lagoon Start", "gondolaBoat"], ["Stone Bridge", "stoneBridge"], ["Canal Turn", "cafe"], ["Canal Finish", "checkpoint"]],
    iceland: [["Lava Coast", "lavaRock"], ["Glacier Wall", "glacierWall"], ["Ice Tunnel", "tunnel"], ["Storm Finish", "checkpoint"]],
    monaco: [["Harbor Hairpin", "marinaYacht"], ["Cliff Tunnel", "tunnel"], ["Casino Square", "cliffVilla"], ["Grand Prix Finish", "checkpoint"]],
    rio: [["Beachfront", "beachKiosk"], ["Hillside Switchback", "hillsideHomes"], ["Mountain Tunnel", "tunnel"], ["Coast Finish", "checkpoint"]]
  };
...
function drawRouteStageLandmarks(w, h, theme) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const center = perspectiveRoadCenter(w, t);
    const sideOffset = w * (phoneMode ? (0.34 + t * 0.64 + seededUnit(seed, i + 10) * 0.1) : (0.22 + t * 0.52 + seededUnit(seed, i + 10) * 0.08));
    const x = center + side * sideOffset;
    const scale = 0.44 + t * (phoneMode ? 1.08 : 1.3);
...
function drawRouteStageProp(prop, x, y, scale, side, design, theme, seed) {
  ...
  if (prop === "tower" || prop === "neon" || prop === "village") {
  ...
  } else if (prop === "hotelBlock" || prop === "glassTower" || prop === "cliffVilla" || prop === "hillsideHomes") {
    if (prop === "hillsideHomes") {
      for (let row = 0; row < 3; row += 1) {
        for (let col = -2; col <= 2; col += 1) {
          ctx.fillStyle = row % 2 ? "rgba(238,216,172,0.86)" : "rgba(198,226,206,0.84)";
          roundRect(col * 20 - 8 + row * 7 * side, -18 - row * 22 - Math.abs(col) * 3, 16, 16, 3);
          ctx.fill();
        }
      }
    } else {
      const bw = prop === "glassTower" ? 46 : prop === "hotelBlock" ? 68 : 58;
      const bh = prop === "glassTower" ? 142 : prop === "hotelBlock" ? 114 : 78;
      ctx.fillStyle = prop === "glassTower" ? "rgba(22,68,76,0.9)" : prop === "hotelBlock" ? "rgba(18,10,32,0.92)" : "rgba(226,218,190,0.88)";
      roundRect(-bw / 2, -bh, bw, bh, 5);
      ctx.fill();
      ctx.fillStyle = prop === "glassTower" ? "rgba(244,251,248,0.48)" : `${accent}88`;
      for (let i = 0; i < 5; i += 1) roundRect(-bw * 0.28, -bh + 14 + i * (bh / 6), bw * 0.56, 4, 2);
      ctx.fill();
    }
  } else if (prop === "marinaYacht" || prop === "gondolaBoat" || prop === "stoneBridge") {
    if (prop === "stoneBridge") {
      ctx.strokeStyle = "rgba(190,178,150,0.86)";
      ctx.lineWidth = 6;
      ctx.beginPath();
      ctx.moveTo(-70, -10);
      ctx.quadraticCurveTo(0, -72, 70, -10);
      ctx.stroke();
      ctx.fillStyle = "rgba(20,64,72,0.32)";
      ctx.beginPath();
      ctx.ellipse(0, 4, 70, 14, 0, 0, Math.PI * 2);
      ctx.fill();
    } else {
      ctx.fillStyle = prop === "gondolaBoat" ? "rgba(22,18,14,0.92)" : "rgba(244,251,248,0.86)";
      ctx.beginPath();
      ctx.moveTo(-66, -16);
      ctx.quadraticCurveTo(-24, 14, 0, 14);
      ctx.quadraticCurveTo(32, 14, 68, -16);
      ctx.lineTo(44, 5);
      ctx.lineTo(-44, 5);
      ctx.closePath();
      ctx.fill();
  ...
  } else if (prop === "glacierWall" || prop === "lavaRock" || prop === "mountainWall") {
    ctx.fillStyle = prop === "glacierWall" ? "rgba(188,224,226,0.82)" : "rgba(96,72,58,0.88)";
    ctx.beginPath();
    ctx.moveTo(-70, 14);
    ctx.lineTo(-38, -56);
    ctx.lineTo(-12, -36);
    ...
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = prop === "glacierWall" ? "rgba(244,251,248,0.6)" : "rgba(255,91,107,0.36)";
    ctx.beginPath();
    ctx.moveTo(-8, -58);
    ctx.lineTo(18, -86);
    ctx.lineTo(34, -32);
    ctx.closePath();
    ctx.fill();
  } else if (prop === "beachKiosk" || prop === "marketStall" || prop === "spectatorDeck") {
    ctx.fillStyle = "rgba(5,8,7,0.84)";
    roundRect(-62, -36, 124, 36, 5);
    ctx.fill();
    ctx.fillStyle = prop === "beachKiosk" ? `${light}bb` : `${accent}bb`;
    if (prop === "spectatorDeck") {
      for (let i = -5; i <= 5; i += 1) {
        ctx.beginPath();
        ctx.arc(i * 11, -44 - Math.abs(i % 2) * 4, 4, 0, Math.PI * 2);
        ctx.fill();
      }
    } else {
      ctx.beginPath();
...
function drawRealWorldDetailPass(w, h, theme) {
  ...
  if (worldMode === "air" || worldMode === "water") {
    drawRealWorldWeatherDisasterPass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed);
  ...
  drawRoadSceneContactShoulders(w, h, horizon, roadTop, roadBottom, place, design, theme);
  ...
  drawGroundLockedWorldVisibilityPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed);
  drawRealWorldRouteDramaPass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed);
  drawHighVisibilityRealWorldDetailPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed);
  drawRoadScenePavementSettlePass(w, h, horizon, roadTop, roadBottom, place);
  ctx.restore();
}
...
function drawGroundLockedWorldVisibilityPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed) {
  ...
  drawRouteChapterLandmarkPass(w, h, horizon, roadTop, roadBottom, place, stage, route, design, theme, seed, phoneMode);
...
function drawHighVisibilityRealWorldDetailPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed) {
  ...
  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 18 : 28);
  for (let i = 0; i < rows; i += 1) {
    ...
    const projectedX = perspectiveRoadCenter(w, t) + side * (half * (phoneMode ? 1.14 : 1.14) + w * (phoneMode ? 0.055 : 0.072));
    const x = roadsideAnchorX(w, h, horizon, roadTop, roadBottom, y, side, projectedX, phoneMode, phoneMode ? 22 : 34);
    const near = Math.max(0.12, Math.min(1, (y - horizon) / Math.max(1, h - horizon)));
    const scale = Math.max(phoneMode ? 0.58 : 0.66, Math.min(phoneMode ? 1.34 : 1.72, (0.76 + near * (phoneMode ? 0.62 : 0.82)) * (0.94 + seededUnit(seed, i + 712) * 0.16)));
...
function highVisibilityDetailSet(place, stage) {
  if (stage && stage.prop === "bridge") return ["coastRail", "life:pierPeople", "lamp", "waterEdge", "parkedCar", "life:sidewalkQueue"];
  if (stage && stage.prop === "tunnel") return ["tunnelPortal", "wallLamp", "lowBarrier", "life:cameraCrew", "roadsideHedge"];
  const sets = {
    coast: ["palm", "beachHouse", "life:beachJoggers", "life:cyclistLane", "coastRail", "life:pierPeople", "lighthouse"],
    ...
    farm: ["barn", "roadsideHedge", "tractorParked", "life:pastureAnimals", "hayBales", "life:fieldWorkers", "windmill"],
    ...
    rainforest: ["canopyTree", "marketStand", "roadsideHedge", "life:marketLife", "life:canopyAnimals", "waterfallSign"],
    europe: ["villageHouse", "cafe", "stoneWall", "life:patioCrowd", "life:cyclistLane", "life:pastureAnimals", "tramPost"],
    vegas: ["hotelBlock", "neonShop", "streetLight", "life:neonCrowd", "parkedCar", "spectatorDeck", "pedestrianBridge"],
    dubai: ["glassTower", "marinaYacht", "streetLight", "life:pierPeople", "pedestrianBridge", "life:cameraCrew", "marketStall"],
    venice: ["stoneBridge", "gondolaBoat", "marinaDock", "life:pierPeople", "cafe", "life:patioCrowd", "stoneWall"],
    iceland: ["glacierWall", "lavaRock", "snowFence", "life:rallyCrew", "supportTent", "life:wildlifeLookout", "curveBoard"],
    monaco: ["cliffVilla", "marinaYacht", "stoneWall", "life:patioCrowd", "life:cameraCrew", "pedestrianBridge", "spectatorDeck"],
    rio: ["hillsideHomes", "beachKiosk", "palm", "life:beachJoggers", "marketStall", "life:marketLife", "roadsideHedge"]
  };
  return sets[place] || sets.city;
}
...
function drawHighVisibilityGroundCue(x, y, scale, side, place, design, theme, seed) {
  ...
  ctx.beginPath();
  const phoneMode = phoneGraphicsActive();
  ctx.ellipse(side * 6, 16, phoneMode ? 82 : 88, phoneMode ? 13 : 15, -0.04 * side, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = phoneMode ? "rgba(0,0,0,0.24)" : "rgba(0,0,0,0.2)";
  ctx.beginPath();
  ctx.ellipse(side * 8, 18, phoneMode ? 68 : 74, phoneMode ? 10 : 10, -0.04 * side, 0, Math.PI * 2);
  ctx.fill();
  if (!phoneMode && !routePrefersLowSignage(place)) {
    ctx.strokeStyle = `${accent}44`;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(-58 * side, 5);
    ctx.lineTo(62 * side, -1);
    ctx.stroke();
  }
  ctx.restore();
}

function drawRealWorldRouteDramaPass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed) {
  const phoneMode = phoneGraphicsActive();
  const length = Math.max(1, raceLength());
  const progress = Math.max(0, Math.min(1, raceState.distance / length));
  const stage = routeStageInfo(place, progress);
  const turn = raceState.roadTurn || raceState.roadCurve || 0;
  const routeKind = realWorldRouteDramaKind(place, stage, turn);
  ctx.save();
  if (routeKind === "tunnel") {
    drawVisibleTunnelDriveThrough(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, phoneMode);
  } else if (routeKind === "mountain" || routeKind === "hill") {
    drawVisibleMountainHillDrive(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, turn, phoneMode);
  } else if (routeKind === "cityTunnel") {
    drawVisibleCityUnderpass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, turn, phoneMode);
  }
  drawVisibleRoadTurnCues(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, turn, phoneMode);
  ctx.restore();
  ctx.globalAlpha = 1;
}

function realWorldRouteDramaKind(place, stage, turn) {
  if (stage && stage.prop === "tunnel") return place === "city" || place === "tokyo" || place === "pursuit" ? "cityTunnel" : "tunnel";
  if (stage && (stage.prop === "mountain" || stage.prop === "cliff")) return "mountain";
  if (stage && (stage.prop === "glacierWall" || stage.prop === "lavaRock" || stage.prop === "cliffVilla" || stage.prop === "hillsideHomes")) return "mountain";
  if (stage && (stage.prop === "hotelBlock" || stage.prop === "glassTower")) return Math.abs(turn) > 0.48 ? "cityTunnel" : "";
  if (stage && (stage.prop === "stoneBridge" || stage.prop === "marinaYacht" || stage.prop === "gondolaBoat")) return Math.abs(turn) > 0.5 ? "hill" : "";
  if (place === "alpine" || place === "europe" || place === "canyon") return Math.abs(turn) > 0.4 ? "mountain" : "hill";
  if (place === "iceland" || place === "monaco" || place === "rio") return Math.abs(turn) > 0.32 ? "mountain" : "hill";
  if (place === "vegas" || place === "dubai") return Math.abs(turn) > 0.58 ? "cityTunnel" : "";
  if (place === "venice") return Math.abs(turn) > 0.62 ? "hill" : "";
  if (place === "desert" || place === "rainforest") return Math.abs(turn) > 0.5 ? "hill" : "";
  return Math.abs(turn) > 0.72 ? "hill" : "";
}

function drawVisibleTunnelDriveThrough(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, phoneMode) {
  const topY = horizon + h * (phoneMode ? 0.02 : 0.01);
  const mouthW = w * (phoneMode ? 0.72 : 0.62);
  const mouthH = h * (phoneMode ? 0.31 : 0.36);
  const x = w * 0.5 + Math.sin(seed + raceState.distance * 0.0002) * w * 0.035;
  ctx.globalAlpha = phoneMode ? 0.7 : 0.68;
  ctx.fillStyle = "rgba(4,6,6,0.72)";
  ctx.beginPath();
  ctx.ellipse(x, topY + mouthH * 0.5, mouthW * 0.5, mouthH * 0.58, 0, Math.PI, 0);
  ctx.lineTo(x + mouthW * 0.5, topY + mouthH * 0.58);
  ctx.lineTo(x - mouthW * 0.5, topY + mouthH * 0.58);
  ctx.closePath();
  ctx.fill();
  ctx.strokeStyle = "rgba(205,218,214,0.52)";
  ctx.lineWidth = Math.max(4, w * 0.01);
  ctx.stroke();

  clipRealWorldRoad(w, h, horizon, roadTop, roadBottom);
  const tunnelShade = ctx.createLinearGradient(0, horizon, 0, h);
  tunnelShade.addColorStop(0, "rgba(0,0,0,0.34)");
  tunnelShade.addColorStop(0.5, "rgba(0,0,0,0.18)");
  tunnelShade.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = tunnelShade;
  ctx.fillRect(0, horizon, w, h - horizon);
  ctx.globalCompositeOperation = "screen";
  for (let i = 0; i < (phoneMode ? 8 : 12); i += 1) {
    const y = horizon + (((i * 86 + raceState.roadOffset * 0.82 + seed * 0.01) % (h - horizon + 160)) - 48);
    if (y < horizon || y > h) continue;
    const t = Math.max(0.02, Math.min(1, (y - horizon) / Math.max(1, h - horizon)));
    const center = perspectiveRoadCenter(w, t);
    const half = realWorldRoadHalf(roadTop, roadBottom, t) * 0.82;
    ctx.strokeStyle = `${design.light || theme[2]}55`;
    ctx.lineWidth = Math.max(2, 2 + t * 8);
    ctx.beginPath();
    ctx.moveTo(center - half * 0.62, y);
    ctx.lineTo(center + half * 0.62, y + t * 8);
    ctx.stroke();
  }
  ctx.globalCompositeOperation = "source-over";
}

function drawVisibleCityUnderpass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, turn, phoneMode) {
  const y = horizon + h * 0.05;
  const x = w * 0.5 + turn * w * 0.05;
  ctx.globalAlpha = phoneMode ? 0.64 : 0.7;
  ctx.fillStyle = "rgba(5,8,7,0.68)";
  roundRect(x - w * 0.34, y - h * 0.055, w * 0.68, h * 0.09, 8);
  ctx.fill();
  ctx.strokeStyle = `${design.accent || theme[1]}88`;
  ctx.lineWidth = Math.max(2, w * 0.005);
  ctx.stroke();
  ctx.fillStyle = "rgba(244,251,248,0.22)";
  for (let i = -3; i <= 3; i += 1) {
    roundRect(x + i * w * 0.08 - w * 0.018, y - h * 0.028, w * 0.036, h * 0.012, 4);
    ctx.fill();
  }
}
...
function drawVisibleMountainHillDrive(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, turn, phoneMode) {
  ...
  for (let side = -1; side <= 1; side += 2) {
    ctx.beginPath();
    ctx.moveTo(side < 0 ? -w * 0.08 : w * 1.08, horizon + h * 0.26);
    ctx.quadraticCurveTo(w * (0.22 + side * 0.12) + turn * w * 0.1, horizon - h * 0.12, w * (0.48 + side * 0.16), horizon + h * 0.16);
    ctx.quadraticCurveTo(w * (0.34 + side * 0.12), horizon + h * 0.26, side < 0 ? -w * 0.08 : w * 1.08, horizon + h * 0.42);
    ctx.closePath();
    ctx.fill();
  }
  clipRealWorldRoad(w, h, horizon, roadTop, roadBottom);
  const hill = ctx.createLinearGradient(0, horizon, 0, h);
  hill.addColorStop(0, "rgba(244,251,248,0.1)");
  hill.addColorStop(0.38, turn > 0 ? "rgba(0,0,0,0.04)" : "rgba(244,251,248,0.08)");
  hill.addColorStop(0.72, "rgba(0,0,0,0.16)");
  hill.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = hill;
  ctx.fillRect(0, horizon, w, h - horizon);
}

function drawVisibleRoadTurnCues(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, turn, phoneMode) {
  if (Math.abs(turn) < 0.22) return;
  ctx.save();
  clipRealWorldRoad(w, h, horizon, roadTop, roadBottom);
  const side = turn > 0 ? 1 : -1;
  const count = phoneMode ? 3 : 7;
  for (let i = 0; i < count; i += 1) {
    const y = horizon + h * (0.22 + i * (phoneMode ? 0.12 : 0.105)) + (((raceState.roadOffset || 0) * 0.28) % 34);
    if (y > h + 30) continue;
    const t = Math.max(0.05, Math.min(1, (y - horizon) / Math.max(1, h - horizon)));
    const center = perspectiveRoadCenter(w, t) + turn * w * 0.08 * (1 - t);
    const half = realWorldRoadHalf(roadTop, roadBottom, t) * (0.24 + t * 0.2);
    ctx.globalAlpha = phoneMode ? Math.min(0.22, 0.08 + t * 0.14) : Math.min(0.48, 0.18 + t * 0.28);
    ctx.strokeStyle = phoneMode ? "rgba(244,251,248,0.72)" : `${design.light || theme[2]}cc`;
    ctx.lineWidth = Math.max(2, 2 + t * (phoneMode ? 3 : 6));
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(center - side * half, y);
    ctx.quadraticCurveTo(center + side * half * 0.25, y + 18 * t, center + side * half, y + 6 + t * 14);
    ctx.stroke();
    if (!phoneMode) {
      ctx.fillStyle = `${design.light || theme[2]}aa`;
      ctx.beginPath();
      ctx.moveTo(center + side * (half + 14 * t), y + 6 + t * 14);
      ctx.lineTo(center + side * (half - 16 * t), y - 5 + t * 8);
      ctx.lineTo(center + side * (half - 8 * t), y + 22 + t * 12);
      ctx.closePath();
      ctx.fill();
    }
  }
...
function drawWaterRaceWorldViewPass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed, phoneMode) {
  ...
  ctx.globalCompositeOperation = "source-over";
  for (let i = 0; i < (phoneMode ? 6 : 14); i += 1) {
  ...
  for (let i = 0; i < (phoneMode ? 4 : 7); i += 1) {
  ...
  for (let i = -1; i < (phoneMode ? 7 : 14); i += 1) {
...
function drawWaterBuoy(x, y, scale, side, theme, seed) {
  ...
  ctx.fill();
  if (phoneMode) {
    const color = side < 0 ? "#ff7a8c" : "#ffd166";
    ctx.globalAlpha = 0.82;
    ctx.fillStyle = color;
    roundRect(-24, -8, 48, 18, 9);
    ctx.fill();
    ctx.fillStyle = "rgba(244,251,248,0.78)";
    roundRect(-16, -4, 32, 6, 3);
    ctx.fill();
    ctx.strokeStyle = "rgba(70,217,255,0.46)";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.ellipse(0, 14, 36, 9, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
    return;
  }
...
function drawVisibleGroundRoadPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, motion, phoneMode) {
  ...
  roadWash.addColorStop(0, phoneMode ? "rgba(26,32,31,0.7)" : "rgba(26,32,31,0.58)");
  roadWash.addColorStop(0.52, phoneMode ? "rgba(8,12,12,0.78)" : "rgba(8,12,12,0.66)");
  roadWash.addColorStop(1, phoneMode ? "rgba(0,0,0,0.92)" : "rgba(0,0,0,0.84)");
  ...
  for (let side = -1; side <= 1; side += 2) {
    if (phoneMode) {
      ...
      for (let i = 0; i < 3; i += 1) {
        ...
        const half = realWorldRoadHalf(roadTop, roadBottom, t);
        const x = perspectiveRoadCenter(w, t) + side * half * 0.72;
        ctx.beginPath();
        ctx.moveTo(x - side * (8 + t * 12), y);
        ctx.lineTo(x + side * (34 + t * 22), y + 2 + t * 3);
        ctx.stroke();
      }
      continue;
    }
  ...
  const laneAlpha = phoneMode ? 0.42 : 0.58;
  ctx.globalCompositeOperation = "screen";
  const visibleLanes = phoneMode ? [-0.5, 0.5] : [-1.5, -0.5, 0.5, 1.5];
  visibleLanes.forEach((lane) => {
    for (let i = 0; i < (phoneMode ? 8 : 16); i += 1) {
      ...
      const dashH = 14 + t * (phoneMode ? 44 : 74);
      const dashW = 5 + t * (phoneMode ? 7 : 12);
      ctx.globalAlpha = laneAlpha * (0.42 + t * 0.58);
      drawPerspectivePavementQuad(w, horizon, lane, y, dashH, dashW, "rgba(244,251,248,0.86)");
    }
  });
  ctx.globalCompositeOperation = "source-over";
...
function drawReferenceInspiredDriveByScenePass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed, phoneMode) {
  ...
  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 24 : 36);
  ...
  scenes.forEach((scene) => {
    ...
    const x = roadsideAnchorX(w, h, horizon, roadTop, roadBottom, scene.y, scene.side, scene.x, phoneMode, phoneMode ? 50 : 76);
    const scale = Math.max(0.34, Math.min(phoneMode ? 1.52 : 1.72, scene.scale * (0.84 + near * 0.58)));
    ctx.globalAlpha = Math.min(0.98, (phoneMode ? 0.72 : 0.56) + near * 0.28);
...
function drawReferenceSceneGroundPatch(kind, side, place, design, theme) {
  const ground = place === "desert" || place === "canyon" ? "rgba(178,92,48,0.26)"
    : place === "coast" || place === "harbor" ? (phoneGraphicsActive() ? "rgba(20,48,54,0.12)" : "rgba(20,48,54,0.13)")
      : place === "farm" ? "rgba(72,94,48,0.14)"
        : place === "snow" || place === "alpine" ? "rgba(244,251,248,0.2)"
          : "rgba(16,24,23,0.26)";
  const phoneMode = phoneGraphicsActive();
  ctx.fillStyle = "rgba(0,0,0,0.36)";
  ctx.beginPath();
  ctx.ellipse(side * -8, 18, phoneMode ? 82 : 94, phoneMode ? 13 : 14, -0.04 * side, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = ground;
  ctx.beginPath();
  const near = phoneMode ? 82 : 96;
  const far = phoneMode ? 96 : 112;
  ctx.moveTo(-near * side, 14);
  ctx.lineTo(far * side, 4);
  ctx.lineTo((far + 14) * side, 28);
  ctx.lineTo(-(near - 8) * side, 34);
  ctx.closePath();
  ctx.fill();
}
...
function drawReferenceGuardrail(side, color) {
  if (phoneGraphicsActive()) {
    ctx.strokeStyle = "rgba(214,226,221,0.32)";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(-58 * side, -4);
    ctx.quadraticCurveTo(0, -10, 60 * side, -6);
    ctx.moveTo(-52 * side, 8);
    ctx.quadraticCurveTo(0, 3, 54 * side, 5);
    ctx.stroke();
    ctx.fillStyle = "rgba(0,0,0,0.2)";
    ctx.beginPath();
    ctx.ellipse(0, 12, 68, 10, -0.03 * side, 0, Math.PI * 2);
    ctx.fill();
    return;
  }
  ...
  if (!phoneGraphicsActive()) {
    ...
    ctx.moveTo(-84 * side, -24);
    ctx.quadraticCurveTo(0, -34, 88 * side, -28);
  }
  ctx.stroke();
  if (phoneGraphicsActive()) {
    ctx.fillStyle = "rgba(0,0,0,0.22)";
    ctx.beginPath();
    ctx.ellipse(0, 9, 106, 12, -0.04 * side, 0, Math.PI * 2);
    ctx.fill();
  }
}
...
function groundLockedPlaceProps(place, stage) {
  ...
  const props = {
    ...
    vegas: ["hotelBlock", "neonShop", "streetLight", "parkedCar", "spectatorDeck"],
    dubai: ["glassTower", "streetLight", "marinaYacht", "pedestrianBridge", "spectators"],
    venice: ["stoneBridge", "marinaDock", "gondolaBoat", "cafe", "spectators"],
    iceland: ["glacierWall", "lavaRock", "snowFence", "supportTent", "spectators"],
    monaco: ["cliffVilla", "stoneWall", "marinaYacht", "streetLight", "spectators"],
    rio: ["hillsideHomes", "beachKiosk", "palm", "marketStall", "spectators"]
  };
...
function routeDriveByProps(place, stage) {
  const stageProps = {
    ...
    hotelBlock: ["hotelBlock", "neonShop", "spectatorDeck", "parkedCar", "streetLight"],
    glassTower: ["glassTower", "marinaYacht", "streetLight", "pedestrianBridge", "spectators"],
    marinaYacht: ["marinaYacht", "dockLight", "stoneWall", "spectators", "routeSign"],
    gondolaBoat: ["gondolaBoat", "stoneBridge", "marinaDock", "cafe", "spectators"],
    stoneBridge: ["stoneBridge", "gondolaBoat", "bridgeRail", "cafe", "dockLight"],
    glacierWall: ["glacierWall", "snowFence", "supportTent", "spectators", "curveBoard"],
    lavaRock: ["lavaRock", "glacierWall", "snowFence", "supportTent", "spectators"],
    cliffVilla: ["cliffVilla", "stoneWall", "marinaYacht", "spectators", "streetLight"],
    hillsideHomes: ["hillsideHomes", "beachKiosk", "palm", "marketStall", "spectators"],
    beachKiosk: ["beachKiosk", "palm", "marketStall", "spectators", "routeSign"]
  };
  ...
  const props = {
    ...
    vegas: ["hotelBlock", "neonShop", "pedestrianBridge", "spectatorDeck", "parkedCar", "crowd"],
    dubai: ["glassTower", "marinaYacht", "pedestrianBridge", "storefront", "trafficSignal", "spectators"],
    venice: ["stoneBridge", "gondolaBoat", "marinaDock", "cafe", "dockLight", "spectators"],
    iceland: ["glacierWall", "lavaRock", "snowFence", "supportTent", "curveBoard", "spectators"],
    monaco: ["cliffVilla", "stoneWall", "marinaYacht", "pedestrianBridge", "billboard", "spectators"],
    rio: ["hillsideHomes", "beachKiosk", "marketStall", "palm", "crowd", "routeSign"]
  };
  return props[place] || props.city;
}
...
function routePrefersLowSignage(place = "") {
  return ["vegas", "dubai", "venice", "iceland", "monaco", "rio"].includes(place);
}
...
function realWorldGroundFallback(place = "city", index = 0) {
  const sets = {
    ...
    europe: ["stoneWall", "cafe", "tramPost", "villageHouse", "spectators"],
    vegas: ["hotelBlock", "neonShop", "streetLight", "parkedCar", "spectatorDeck"],
    dubai: ["glassTower", "streetLight", "marinaYacht", "storefront", "spectators"],
    venice: ["stoneBridge", "gondolaBoat", "marinaDock", "cafe", "spectators"],
    iceland: ["glacierWall", "lavaRock", "snowFence", "supportTent", "spectators"],
    monaco: ["cliffVilla", "stoneWall", "marinaYacht", "streetLight", "spectators"],
    rio: ["hillsideHomes", "beachKiosk", "marketStall", "palm", "crowd"]
  };
...
function drawRouteAnchoredDriveByWorldPass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed, phoneMode) {
  ...
  const lookAhead = phoneMode ? 2100 : 2600;
  const first = Math.floor((raceState.distance - lookBehind) / spacing) - 1;
  const last = Math.ceil((raceState.distance + lookAhead) / spacing) + 1;
  const drawList = [];
  for (let index = first; index <= last; index += 1) {
    ...
    const lane = side * (2.46 + seededUnit(seed, index + 33) * 0.62);
    const projected = roadObjectPos(lane, baseDistance);
    ...
    if (!phoneMode && index % 5 === 0) {
      const otherSide = -side;
      const companionLane = otherSide * (2.38 + seededUnit(seed, index + 44) * 0.52);
  ...
  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 18 : 26);
  const laneBands = { "-1": -9999, "1": -9999 };
  let recentSignY = -9999;
  drawList.forEach((item) => {
    const sideKey = String(item.side < 0 ? -1 : 1);
    const minGap = phoneMode ? (isSignLikeWorldProp(item.kind) ? 210 : 138) : (isSignLikeWorldProp(item.kind) ? 146 : 88);
    ...
    const x = roadsideAnchorX(w, h, horizon, roadTop, roadBottom, item.y, item.side, item.x, phoneMode, phoneMode ? 26 : 42);
    const scale = Math.max(0.36, Math.min(phoneMode ? 1.72 : 1.82, item.scale * (phoneMode ? 1.03 : 1) * (0.9 + near * 0.52)));
    ...
    ctx.globalAlpha = Math.min(1, (phoneMode ? 0.94 : 0.88) + near * 0.2);
    drawGroundLockedPlaceProp(item.kind, x, item.y, scale, item.side, place, route, design, theme, item.seed);
  });
  ctx.restore();
  ctx.globalAlpha = 1;
}

function routeChapterLandmarkSet(place, stage) {
  if (stage && stage.prop === "checkpoint") return ["checkpointArch", "spectatorDeck", "streetLight"];
  if (stage && stage.prop === "tunnel") return ["tunnelPortal", "wallLamp", "lowBarrier"];
  const stageSets = {
    hotelBlock: ["hotelBlock", "neonShop", "spectatorDeck"],
    glassTower: ["glassTower", "marinaYacht", "pedestrianBridge"],
    marinaYacht: ["marinaYacht", "dockLight", "spectatorDeck"],
    gondolaBoat: ["gondolaBoat", "stoneBridge", "cafe"],
    stoneBridge: ["stoneBridge", "gondolaBoat", "marinaDock"],
    glacierWall: ["glacierWall", "snowFence", "supportTent"],
    lavaRock: ["lavaRock", "glacierWall", "curveBoard"],
    cliffVilla: ["cliffVilla", "marinaYacht", "stoneWall"],
    hillsideHomes: ["hillsideHomes", "beachKiosk", "marketStall"],
    beachKiosk: ["beachKiosk", "palm", "marketStall"],
    mountain: ["mountainWall", "pine", "stoneWall"],
    bridge: ["stoneBridge", "bridgeRail", "spectators"],
    pier: ["marinaDock", "marinaYacht", "dockLight"]
  };
  if (stage && stageSets[stage.prop]) return stageSets[stage.prop];
  const routeSets = {
    vegas: ["hotelBlock", "neonShop", "spectatorDeck"],
    dubai: ["glassTower", "marinaYacht", "pedestrianBridge"],
    venice: ["stoneBridge", "gondolaBoat", "cafe"],
    iceland: ["glacierWall", "lavaRock", "snowFence"],
    monaco: ["cliffVilla", "marinaYacht", "spectatorDeck"],
    rio: ["hillsideHomes", "beachKiosk", "marketStall"],
    coast: ["beachHouse", "palm", "coastRail"],
    city: ["storefront", "parkedCar", "streetLight"],
    canyon: ["rockFace", "curveBoard", "tourVan"],
    alpine: ["chalet", "pine", "stoneWall"],
    harbor: ["warehouse", "container", "marinaDock"],
    snow: ["lodge", "snowFence", "pine"],
    freight: ["truckStop", "serviceTruck", "overheadSign"],
    farm: ["barn", "tractorParked", "windmill"],
    tokyo: ["neonShop", "pedestrianBridge", "streetLight"],
    desert: ["supportTent", "dune", "rallyMarker"],
    rainforest: ["marketStand", "canopyTree", "woodRail"],
    europe: ["villageHouse", "cafe", "stoneWall"]
  };
  return routeSets[place] || routeSets.city;
}

function drawRouteChapterLandmarkPass(w, h, horizon, roadTop, roadBottom, place, stage, route, design, theme, seed, phoneMode) {
  const length = Math.max(1, raceLength());
  const spacing = phoneMode ? 940 : 780;
  const lookBehind = phoneMode ? 260 : 360;
  const lookAhead = phoneMode ? 2600 : 3200;
  const first = Math.floor((raceState.distance - lookBehind) / spacing) - 1;
  const last = Math.ceil((raceState.distance + lookAhead) / spacing) + 1;
  const clusters = [];
  for (let index = first; index <= last; index += 1) {
    const baseDistance = index * spacing + seededUnit(seed, index + 202) * spacing * 0.24;
    if (baseDistance < 0 || baseDistance > length + lookAhead * 0.2) continue;
    const gap = baseDistance - raceState.distance;
    if (gap < -lookBehind || gap > lookAhead) continue;
    const progress = Math.max(0, Math.min(1, baseDistance / length));
    const routeStage = routeStageInfo(place, progress);
    const set = routeChapterLandmarkSet(place, routeStage);
    const side = seededUnit(seed, index + 211) > 0.5 ? 1 : -1;
    const lane = side * (2.84 + seededUnit(seed, index + 219) * (phoneMode ? 0.5 : 0.72));
    const pos = roadObjectPos(lane, baseDistance);
    if (!pos || pos.y < horizon - 60 || pos.y > h + 140) continue;
    clusters.push({
      x: pos.x,
      y: pos.y,
      scale: pos.scale,
      depth: pos.depth || Math.max(0, Math.min(1, (pos.y - horizon) / Math.max(1, h - horizon))),
      side,
      set,
      stage: routeStage,
      seed: seed + index * 173,
  ...
  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 32 : 46);
  clusters.sort((a, b) => a.y - b.y).forEach((cluster, clusterIndex) => {
    const near = Math.max(0.1, Math.min(1, cluster.depth || 0.5));
    const anchorX = roadsideAnchorX(w, h, horizon, roadTop, roadBottom, cluster.y, cluster.side, cluster.x, phoneMode, phoneMode ? 42 : 62);
    const baseScale = Math.max(phoneMode ? 0.66 : 0.72, Math.min(phoneMode ? 1.58 : 1.9, cluster.scale * (1.08 + near * 0.58)));
    const alpha = Math.min(1, phoneMode ? 0.86 + near * 0.14 : 0.78 + near * 0.2);
    ctx.globalAlpha = alpha;
    drawRouteChapterGroundPad(anchorX, cluster.y, baseScale, cluster.side, place, design, theme);
    cluster.set.slice(0, phoneMode ? 2 : 3).forEach((kind, itemIndex) => {
      const offset = (itemIndex - 0.7) * (phoneMode ? 82 : 92) * baseScale * cluster.side;
      const yOffset = itemIndex === 0 ? 0 : (phoneMode ? -8 : -12) * baseScale;
      const localScale = baseScale * (itemIndex === 0 ? 1.02 : 0.78 - itemIndex * 0.04);
      drawGroundLockedPlaceProp(kind, anchorX + offset, cluster.y + yOffset, localScale, cluster.side, place, route, design, theme, cluster.seed + itemIndex * 31);
    });
    if (!phoneMode && !routePrefersLowSignage(place) && clusterIndex === 0) {
      drawRouteChapterSmallBoard(anchorX, cluster.y, baseScale, cluster.side, cluster.stage, route, design, theme);
    }
  });
  ctx.restore();
  ctx.globalAlpha = 1;
...
function drawRouteChapterGroundPad(x, y, scale, side, place, design, theme) {
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(scale, scale);
  const ground = place === "venice" || place === "dubai" || place === "monaco" || place === "coast" || place === "harbor"
    ? "rgba(24,68,76,0.28)"
    : place === "rio" || place === "rainforest" || place === "farm"
      ? "rgba(32,104,58,0.26)"
      : place === "iceland" || place === "snow" || place === "alpine"
        ? "rgba(214,230,230,0.28)"
        : place === "vegas" || place === "desert" || place === "canyon"
          ? "rgba(180,104,48,0.24)"
          : "rgba(42,54,52,0.26)";
  ctx.fillStyle = ground;
  ctx.beginPath();
  ctx.ellipse(side * 18, 18, 158, 22, -0.05 * side, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(0,0,0,0.34)";
  ctx.beginPath();
  ctx.ellipse(side * 14, 20, 126, 13, -0.05 * side, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = `${design.accent || theme[1] || "#46d9ff"}44`;
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(-118 * side, 2);
  ctx.lineTo(128 * side, -9);
  ctx.stroke();
  ctx.restore();
}

function drawRouteChapterSmallBoard(x, y, scale, side, stage, route, design, theme) {
  ctx.save();
  ctx.translate(x + side * 142 * scale, y - 48 * scale);
  ctx.scale(scale, scale);
  ctx.fillStyle = "rgba(5,8,7,0.82)";
  roundRect(-72, -24, 144, 34, 5);
  ctx.fill();
  ctx.strokeStyle = `${design.accent || theme[1] || "#46d9ff"}88`;
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.fillStyle = "#f4fbf8";
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText((stage && stage.label ? stage.label : route.scene).toUpperCase(), 0, -9, 118);
  ctx.fillStyle = `${design.light || theme[2] || "#ffd166"}cc`;
  ctx.fillRect(-48, 3, 96, 4);
  ctx.restore();
}
...
function drawGroundLockedDriveByShadow(x, y, scale, side, theme) {
  ...
  ctx.beginPath();
  ctx.ellipse(side * -4, 10, phoneGraphicsActive() ? 66 : 74, phoneGraphicsActive() ? 10 : 11, -0.04 * side, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = `${theme[1] || "#46d9ff"}22`;
  roundRect(phoneGraphicsActive() ? -56 : -64, 0, phoneGraphicsActive() ? 112 : 128, 5, 4);
  ctx.fill();
  ctx.restore();
}
...
function drawGroundLockedSideDetailPass(w, h, horizon, roadTop, roadBottom, place, stage, route, design, theme, seed, motion, phoneMode) {
  ...
  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 14 : 22);
  const lastYBySide = { "-1": -9999, "1": -9999 };
  for (let i = 0; i < count; i += 1) {
    const y = horizon + (((i * (phoneMode ? 230 : 178) + motion * (0.58 + seededUnit(seed, i) * 0.14) + seed * 0.003) % span) - 112);
    if (y < horizon + 4 || y > h + 70) continue;
    const t = Math.max(0.06, Math.min(1.08, (y - horizon) / Math.max(1, h - horizon)));
    const side = i % 2 === 0 ? -1 : 1;
    const half = realWorldRoadHalf(roadTop, roadBottom, t);
    const shoulder = half * (1.24 + seededUnit(seed, i + 40) * 0.2) + w * (0.056 + seededUnit(seed, i + 60) * 0.05);
    const projectedX = perspectiveRoadCenter(w, t) + side * shoulder;
    const x = roadsideAnchorX(w, h, horizon, roadTop, roadBottom, y, side, projectedX, phoneMode, phoneMode ? 20 : 34);
    const scale = (0.4 + t * (phoneMode ? 1.02 : 1.08)) * (0.92 + seededUnit(seed, i + 80) * 0.22);
    const kind = worldSceneryProp(props[i % props.length], place, i, phoneMode);
    const sideKey = String(side);
    const rowGap = phoneMode ? (isSignLikeWorldProp(kind) ? 190 : 126) : (isSignLikeWorldProp(kind) ? 132 : 82);
    if (y - lastYBySide[sideKey] < rowGap) continue;
    lastYBySide[sideKey] = y;
    ctx.globalAlpha = Math.min(1, (phoneMode ? 0.84 : 0.78) + t * 0.28);
...
function drawGroundLockedPlaceProp(kind, x, y, scale, side, place, route, design, theme, seed) {
  ...
  const lowSignage = routePrefersLowSignage(place);
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(scale, scale);
  ...
  if (kind === "streetLight" || kind === "lamp" || kind === "dockLight" || kind === "runwayLight" || kind === "wallLamp") {
  ...
  } else if (kind === "hotelBlock" || kind === "glassTower" || kind === "hillsideHomes" || kind === "cliffVilla") {
    if (kind === "hillsideHomes") {
      for (let row = 0; row < 3; row += 1) {
        for (let col = -2; col <= 2; col += 1) {
          const bx = col * 28 + row * 9 * side;
          const by = -18 - row * 30 - Math.abs(col) * 3;
          ctx.fillStyle = row % 2 ? "rgba(238,216,172,0.92)" : "rgba(210,232,220,0.9)";
          roundRect(bx - 12, by - 20, 24, 20, 3);
          ctx.fill();
          ctx.fillStyle = "rgba(130,46,38,0.9)";
          ctx.beginPath();
          ...
          ctx.fillRect(bx - 5, by - 12, 10, 6);
        }
      }
      ctx.fillStyle = "rgba(28,92,44,0.62)";
      ctx.beginPath();
      ctx.ellipse(0, -2, 106, 20, -0.05 * side, 0, Math.PI * 2);
      ctx.fill();
    } else if (kind === "cliffVilla") {
      ctx.fillStyle = "rgba(126,118,102,0.72)";
      ctx.beginPath();
      ctx.moveTo(-86, 4);
      ctx.lineTo(-44, -82);
      ctx.lineTo(34, -48);
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = "rgba(238,232,208,0.94)";
      roundRect(-54, -74, 108, 58, 5);
      ctx.fill();
      ...
      ctx.fill();
      ctx.fillStyle = "rgba(5,8,7,0.72)";
      for (let i = -1; i <= 1; i += 1) roundRect(i * 30 - 8, -52, 16, 16, 3);
      ctx.fill();
    } else {
      const tower = kind === "glassTower";
      const towerW = tower ? 88 : 134;
      const towerH = tower ? 170 : 126;
      const gradient = ctx.createLinearGradient(-towerW / 2, -towerH, towerW / 2, 0);
      gradient.addColorStop(0, tower ? "rgba(170,230,235,0.48)" : "rgba(28,18,42,0.95)");
      gradient.addColorStop(0.42, tower ? "rgba(18,58,66,0.9)" : "rgba(18,12,34,0.94)");
      gradient.addColorStop(1, "rgba(5,8,7,0.96)");
      ctx.fillStyle = gradient;
      roundRect(-towerW / 2, -towerH, towerW, towerH, tower ? 10 : 7);
      ctx.fill();
      ctx.strokeStyle = tower ? "rgba(244,251,248,0.38)" : `${accent}77`;
      ctx.lineWidth = 3;
      for (let i = -2; i <= 2; i += 1) {
        ctx.beginPath();
        ctx.moveTo(i * towerW * 0.16, -towerH + 12);
        ctx.lineTo(i * towerW * 0.12, -8);
        ctx.stroke();
      }
      for (let row = 0; row < 6; row += 1) {
        ctx.fillStyle = row % 2 ? `${light}55` : `${accent}66`;
        roundRect(-towerW * 0.34, -towerH + 20 + row * (towerH / 7), towerW * 0.68, 5, 3);
        ctx.fill();
      }
      if (kind === "hotelBlock") {
        ctx.fillStyle = "rgba(255,79,216,0.88)";
        roundRect(-60, -towerH - 22, 120, 20, 5);
        ctx.fill();
      }
    }
  } else if (kind === "marinaYacht" || kind === "gondolaBoat" || kind === "shoreBoat") {
    ctx.fillStyle = "rgba(20,64,72,0.62)";
    ctx.beginPath();
    ctx.ellipse(0, 1, 104, 22, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = kind === "gondolaBoat" ? "rgba(25,20,16,0.94)" : "rgba(244,251,248,0.9)";
    ctx.beginPath();
    ctx.moveTo(-78, -18);
    ctx.quadraticCurveTo(-34, 18, 0, 18);
    ctx.quadraticCurveTo(42, 18, 82, -18);
    ctx.lineTo(54, 6);
    ctx.lineTo(-54, 6);
    ctx.closePath();
    ctx.fill();
    if (kind === "marinaYacht") {
      ...
      ctx.strokeStyle = "rgba(214,180,112,0.78)";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(-82, -22);
      ctx.quadraticCurveTo(0, -42, 86, -22);
      ctx.stroke();
    }
  } else if (kind === "glacierWall" || kind === "lavaRock" || kind === "mountainWall") {
    const icy = kind === "glacierWall";
    ctx.fillStyle = icy ? "rgba(188,224,226,0.82)" : "rgba(78,64,58,0.9)";
    ctx.beginPath();
    ctx.moveTo(-100, 4);
    ctx.lineTo(-62, -72);
    ctx.lineTo(-24, -44);
    ...
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = icy ? "rgba(244,251,248,0.72)" : "rgba(255,91,107,0.38)";
    ctx.beginPath();
    ctx.moveTo(-18, -74);
    ctx.lineTo(18, -104);
    ctx.lineTo(42, -48);
    ctx.closePath();
    ctx.fill();
    ctx.strokeStyle = icy ? "rgba(70,217,255,0.44)" : "rgba(255,183,74,0.34)";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(-74, -28);
    ctx.lineTo(72, -12);
    ...
    ctx.lineTo(34, -28);
    ctx.stroke();
  } else if (kind === "stoneBridge") {
    ctx.strokeStyle = "rgba(190,178,150,0.88)";
    ctx.lineWidth = 9;
    ctx.beginPath();
    ctx.moveTo(-104, -14);
    ctx.quadraticCurveTo(0, -96, 104, -14);
    ctx.stroke();
    ctx.strokeStyle = "rgba(244,251,248,0.34)";
    ctx.lineWidth = 4;
    for (let i = -3; i <= 3; i += 1) {
      ctx.beginPath();
      ctx.moveTo(i * 30, -28 - Math.abs(i) * 5);
      ctx.lineTo(i * 26, -4);
      ctx.stroke();
    }
    ctx.fillStyle = "rgba(20,64,72,0.4)";
    ctx.beginPath();
    ctx.ellipse(0, 4, 94, 18, 0, 0, Math.PI * 2);
    ctx.fill();
  } else if (kind === "beachKiosk" || kind === "marketStall" || kind === "spectatorDeck") {
    if (kind === "spectatorDeck") {
      ctx.fillStyle = "rgba(5,8,7,0.86)";
      roundRect(-92, -48, 184, 48, 6);
      ctx.fill();
      ctx.strokeStyle = `${light}aa`;
      ctx.lineWidth = 4;
      ctx.beginPath();
      ...
      for (let i = -6; i <= 6; i += 1) {
        ...
        ctx.beginPath();
        ctx.arc(i * 13, -56 - Math.abs(i % 2) * 4, 5, 0, Math.PI * 2);
        ctx.fill();
      }
    } else {
      ctx.fillStyle = "rgba(36,34,26,0.92)";
      roundRect(-64, -42, 128, 42, 5);
      ctx.fill();
      ctx.fillStyle = kind === "beachKiosk" ? "rgba(255,209,102,0.9)" : "rgba(255,91,107,0.86)";
      ctx.beginPath();
      ctx.moveTo(-74, -42);
      ctx.lineTo(0, -78);
      ctx.lineTo(74, -42);
      ...
      for (let i = -2; i <= 2; i += 1) ctx.fillRect(i * 22 - 6, -36, 12, 18);
    }
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    if (!lowSignage) ctx.fillText(label.toUpperCase(), 0, -108, 162);
    ctx.fillStyle = light;
    ctx.font = "900 10px system-ui";
    if (!lowSignage) ctx.fillText("NEXT SECTOR", 0, -87, 142);
    ctx.fillStyle = `${accent}66`;
    ctx.fillRect(-84, lowSignage ? -108 : -76, 168, lowSignage ? 8 : 5);
    if (lowSignage) {
      ctx.fillStyle = `${light}77`;
      ctx.fillRect(-54, -92, 108, 6);
    }
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    if (!lowSignage || kind === "checkpointArch") ctx.fillText(kind === "checkpointArch" ? "CHECKPOINT" : label.toUpperCase(), 0, -109, 170);
    else {
      ctx.fillStyle = `${accent}88`;
      ctx.fillRect(-70, -112, 140, 6);
      ctx.fillStyle = `${light}77`;
      ctx.fillRect(-40, -100, 80, 5);
    }
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    if (!lowSignage) ctx.fillText(kind === "weighStation" ? "WEIGH STATION" : kind === "dieselSign" ? "DIESEL STOP" : label.toUpperCase(), 0, -111, 188);
    ctx.fillStyle = `${light}99`;
    ctx.fillRect(-92, lowSignage ? -112 : -88, 184, 6);
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    if (!lowSignage) ctx.fillText(label.toUpperCase(), 0, -72, 126);
    ctx.fillStyle = light;
    ctx.fillRect(-54, lowSignage ? -72 : -55, 108, lowSignage ? 6 : 5);
  ...
  } else if (kind === "roadsideHedge" || kind === "coastRail" || kind === "fence" || kind === "snowFence" || kind === "woodRail" || kind === "stoneWall" || kind === "seaWall" || kind === "bridgeRail" || kind === "guardPost" || kind === "lowBarrier" || kind === "curbBlock" || kind === "reflectorPost" || kind === "barrier") {
    ...
    if (kind === "roadsideHedge") {
      const hedge = place === "desert" || place === "canyon" ? "rgba(156,92,42,0.54)" : place === "snow" || place === "alpine" ? "rgba(222,236,232,0.58)" : "rgba(44,132,74,0.58)";
      ctx.fillStyle = hedge;
      ctx.beginPath();
      ctx.ellipse(0, -12, 96, 24, -0.04, 0, Math.PI * 2);
      ctx.ellipse(-44, -23, 42, 18, 0.16, 0, Math.PI * 2);
      ctx.ellipse(44, -24, 44, 18, -0.12, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = "rgba(244,251,248,0.28)";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.moveTo(-52, -24);
      ctx.quadraticCurveTo(0, -31, 54, -25);
      ctx.moveTo(-48, -8);
      ctx.quadraticCurveTo(0, -14, 50, -10);
      ctx.stroke();
      ctx.fillStyle = `${accent}55`;
      ctx.beginPath();
      ctx.ellipse(-30, -21, 8, 4, 0, 0, Math.PI * 2);
      ctx.ellipse(24, -23, 9, 4, 0, 0, Math.PI * 2);
      ctx.fill();
    } else {
      ctx.beginPath();
...
function drawDepthParallaxBands(w, h, horizon, roadTop, roadBottom, place, stage, route, design, theme, seed, motion, phoneMode) {
  ...
  ctx.save();
  ctx.globalAlpha = phoneMode ? 0.58 : 0.42;
...
function drawDepthBandShape(kind, x, y, scale, place, design, theme, seed) {
  ...
  if (kind === "cityBlocks" || kind === "neonTowers") {
    for (let i = -2; i <= 2; i += 1) {
      ...
      ctx.fillStyle = phoneMode ? (kind === "neonTowers" ? "rgba(14,9,24,0.88)" : "rgba(7,15,17,0.82)") : (kind === "neonTowers" ? "rgba(18,10,32,0.62)" : "rgba(8,18,20,0.58)");
    ...
    const color = kind === "duneLayers" ? (phoneMode ? "rgba(255,183,74,0.42)" : "rgba(255,183,74,0.32)") : kind === "rockCuts" ? (phoneMode ? "rgba(178,92,48,0.42)" : "rgba(178,92,48,0.32)") : kind === "villageSlope" ? (phoneMode ? "rgba(120,142,132,0.38)" : "rgba(120,142,132,0.3)") : (phoneMode ? "rgba(160,172,164,0.42)" : "rgba(160,172,164,0.3)");
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.moveTo(-110, 34);
    ...
    ctx.fillStyle = phoneMode ? "rgba(35,86,96,0.34)" : "rgba(70,217,255,0.2)";
    ctx.beginPath();
    ...
    ctx.fillStyle = kind === "canopyLayers" ? (phoneMode ? "rgba(30,104,56,0.4)" : "rgba(30,104,56,0.3)") : kind === "snowVillage" ? (phoneMode ? "rgba(244,251,248,0.34)" : "rgba(244,251,248,0.24)") : (phoneMode ? "rgba(95,126,54,0.38)" : "rgba(95,126,54,0.28)");
    ctx.fillRect(-130, -12, 260, 48);
    ctx.strokeStyle = kind === "snowVillage" ? "rgba(244,251,248,0.28)" : "rgba(187,242,74,0.2)";
    ctx.lineWidth = 2;
    const rows = phoneMode ? 2 : 7;
...
function routeWeatherCondition(place, progress = 0, elapsed = 0) {
  const phase = (Math.floor(progress * 8) + Math.floor(elapsed / 28)) % 4;
  const night = raceIsNight(selectedRace);
  const table = {
    coast: [
      { sky: "sun", surface: "wet", hazard: "spray", label: "sun glare on sea mist" },
      { sky: "cloud", surface: "standingWater", hazard: "coastalFlood", label: "water over low coast road" },
      { sky: "rain", surface: "wet", hazard: "train", label: "rain slick coast highway" },
      { sky: "storm", surface: "standingWater", hazard: "tidalSurge", label: "surging shoreline" }
    ],
    city: [
      { sky: "cloud", surface: "wet", hazard: "train", label: "wet downtown pavement" },
      { sky: "rain", surface: "standingWater", hazard: "train", label: "rain and light puddles" },
      { sky: night ? "night" : "sun", surface: "dry", hazard: "train", label: "clear city traffic" },
      { sky: "cloud", surface: "oilSlick", hazard: "train", label: "oil on city asphalt" }
    ],
    canyon: [
      { sky: "sun", surface: "dry", hazard: "rockfall", label: "dry canyon switchback" },
      { sky: "cloud", surface: "cracked", hazard: "earthquake", label: "quake-cracked canyon road" },
      { sky: "sun", surface: "dust", hazard: "train", label: "dusty canyon rail line" },
      { sky: "storm", surface: "wet", hazard: "rockfall", label: "storm runoff on rock road" }
    ],
    alpine: [
      { sky: "snow", surface: "snowSlush", hazard: "train", label: "snow slush mountain pass" },
      { sky: "cloud", surface: "blackIce", hazard: "train", label: "black ice warning" },
      { sky: "sun", surface: "wet", hazard: "meltwater", label: "sunny meltwater road" },
      { sky: "snow", surface: "snowSlush", hazard: "avalancheView", label: "snowy mountain road" }
    ],
    harbor: [
      { sky: "cloud", surface: "wet", hazard: "train", label: "wet port pavement" },
      { sky: "storm", surface: "standingWater", hazard: "tidalSurge", label: "harbor flood water" },
      { sky: "sun", surface: "oilSlick", hazard: "train", label: "diesel sheen by docks" },
      { sky: "rain", surface: "standingWater", hazard: "coastalFlood", label: "rain over dock road" }
    ],
    snow: [
      { sky: "snow", surface: "snowSlush", hazard: "train", label: "snow-packed road" },
      { sky: "night", surface: "blackIce", hazard: "train", label: "black ice at night" },
      { sky: "cloud", surface: "snowSlush", hazard: "meltwater", label: "slush and meltwater" },
      { sky: "sun", surface: "wet", hazard: "train", label: "bright winter glare" }
    ],
    airfield: [
      { sky: "sun", surface: "dry", hazard: "train", label: "clear runway heat shimmer" },
      { sky: "cloud", surface: "wet", hazard: "crosswind", label: "wet runway" },
      { sky: "storm", surface: "standingWater", hazard: "crosswind", label: "stormy runway spray" },
      { sky: "night", surface: "dry", hazard: "train", label: "night runway lights" }
    ],
    freight: [
      { sky: "cloud", surface: "oilSlick", hazard: "train", label: "diesel-stained freight road" },
      { sky: "rain", surface: "wet", hazard: "train", label: "rainy rail corridor" },
      { sky: "sun", surface: "dry", hazard: "train", label: "sunlit freight bypass" },
      { sky: "cloud", surface: "cracked", hazard: "train", label: "patched industrial pavement" }
    ],
    farm: [
      { sky: "sun", surface: "dry", hazard: "train", label: "clear county road" },
      { sky: "storm", surface: "wet", hazard: "tornado", label: "storm cell over fields" },
      { sky: "cloud", surface: "mud", hazard: "train", label: "mud at farm crossing" },
      { sky: "rain", surface: "standingWater", hazard: "train", label: "rain over low farm road" }
    ],
    tokyo: [
      { sky: "night", surface: "wet", hazard: "train", label: "neon wet expressway" },
      { sky: "rain", surface: "standingWater", hazard: "train", label: "rain reflections and rail" },
      { sky: "night", surface: "oilSlick", hazard: "train", label: "dark glossy service lane" },
      { sky: "cloud", surface: "wet", hazard: "train", label: "cloudy expressway" }
    ],
    desert: [
      { sky: "sun", surface: "dry", hazard: "heat", label: "dry desert highway" },
      { sky: "storm", surface: "dust", hazard: "tornado", label: "dust funnel in distance" },
      { sky: "cloud", surface: "cracked", hazard: "earthquake", label: "broken desert pavement" },
      { sky: "sun", surface: "dust", hazard: "train", label: "rail line through desert" }
    ],
    rainforest: [
      { sky: "rain", surface: "wet", hazard: "train", label: "rainforest wet road" },
      { sky: "storm", surface: "standingWater", hazard: "floodedBridge", label: "flooded low bridge" },
      { sky: "cloud", surface: "mud", hazard: "train", label: "mud washed shoulder" },
      { sky: "sun", surface: "wet", hazard: "steam", label: "sun after rain" }
    ],
    europe: [
      { sky: "cloud", surface: "wet", hazard: "train", label: "wet village road" },
  ...
  const condition = { ...list[phase % list.length] };
  if (night && condition.sky !== "snow" && condition.sky !== "rain" && condition.sky !== "storm") condition.sky = "night";
  return condition;
}

function weatherRoadGrip(surface) {
  const grip = {
    dry: 1,
    dust: 0.94,
    wet: 0.91,
  ...
  return grip[surface] || 1;
}

function weatherRoadSpeed(surface) {
  const speed = {
    dry: 1,
    dust: 0.98,
    wet: 0.97,
...
function drawPlaceIdentityObject(kind, x, y, scale, side, place, design, theme, seed) {
  ...
  ctx.translate(x, y);
  ctx.scale(scale, scale);
  ctx.globalAlpha *= phoneGraphicsActive() ? 0.62 : 0.46;
  ctx.fillStyle = "rgba(0,0,0,0.28)";
  ctx.beginPath();
  ...
  if (kind === "downtownTower" || kind === "glassOffice" || kind === "neonBlock" || kind === "towerSign") {
    ...
    ctx.fill();
    if (phoneMode) {
      ctx.fillStyle = "rgba(130,86,45,0.6)";
      roundRect(-92, -4, 184, 22, 7);
      ctx.fill();
      ctx.strokeStyle = "rgba(244,251,248,0.28)";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(-78 * side, 2);
      ctx.lineTo(80 * side, -8);
      ctx.stroke();
    } else {
...
function drawRoadSceneContactShoulders(w, h, horizon, roadTop, roadBottom, place, design, theme) {
  const phoneMode = phoneGraphicsActive();
  const shoulderColor = place === "coast" || place === "harbor" || place === "venice" || place === "monaco" || place === "dubai" ? "rgba(18,48,54,0.55)"
    : place === "snow" || place === "alpine" || place === "iceland" ? "rgba(214,226,221,0.34)"
      : place === "desert" || place === "canyon" || place === "vegas" ? "rgba(154,88,42,0.42)"
        : place === "farm" || place === "rainforest" || place === "rio" ? "rgba(32,74,48,0.4)"
          : "rgba(8,13,12,0.5)";
  const edgeGlow = design && design.light ? `${design.light}55` : `${theme[2] || "#ffd166"}55`;

  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 5 : 8);
  for (let side = -1; side <= 1; side += 2) {
    const sideGrad = ctx.createLinearGradient(0, horizon, 0, h);
    sideGrad.addColorStop(0, "rgba(0,0,0,0)");
    sideGrad.addColorStop(0.42, shoulderColor);
    sideGrad.addColorStop(1, "rgba(0,0,0,0.62)");
    ctx.strokeStyle = sideGrad;
    ctx.lineWidth = Math.max(phoneMode ? 16 : 20, w * (phoneMode ? 0.034 : 0.026));
    ctx.globalAlpha = phoneMode ? 0.36 : 0.32;
    ctx.beginPath();
    for (let step = 0; step <= 18; step += 1) {
      const t = step / 18;
      const y = horizon + (h - horizon + 18) * t;
      const x = perspectiveRoadCenter(w, t) + side * (realWorldRoadHalf(roadTop, roadBottom, t) + w * 0.012);
    ...
    ctx.globalAlpha = phoneMode ? 0.16 : 0.18;
    ctx.beginPath();
    for (let step = 0; step <= 12; step += 1) {
      const t = step / 12;
      const y = horizon + (h - horizon + 16) * t;
      const x = perspectiveRoadCenter(w, t) + side * (realWorldRoadHalf(roadTop, roadBottom, t) + w * 0.002);
      if (step === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
    ctx.stroke();
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    ctx.lineWidth = Math.max(phoneMode ? 4 : 5, w * 0.006);
    ctx.globalAlpha = phoneMode ? 0.28 : 0.24;
    ctx.beginPath();
    for (let step = 0; step <= 16; step += 1) {
      const t = step / 16;
      const y = horizon + (h - horizon + 18) * t;
      const x = perspectiveRoadCenter(w, t) + side * (realWorldRoadHalf(roadTop, roadBottom, t) - w * 0.006);
      if (step === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
    ctx.stroke();
...
function drawRoadScenePavementSettlePass(w, h, horizon, roadTop, roadBottom, place) {
  const phoneMode = phoneGraphicsActive();
  ctx.save();
  clipRealWorldRoad(w, h, horizon, roadTop, roadBottom);
  const settle = ctx.createLinearGradient(0, horizon, 0, h);
  if (place === "desert" || place === "canyon" || place === "farm" || place === "vegas") {
    settle.addColorStop(0, phoneMode ? "rgba(30,25,20,0.16)" : "rgba(28,24,20,0.22)");
    settle.addColorStop(1, phoneMode ? "rgba(0,0,0,0.42)" : "rgba(0,0,0,0.46)");
  } else {
    settle.addColorStop(0, phoneMode ? "rgba(12,15,15,0.22)" : "rgba(8,11,11,0.44)");
    settle.addColorStop(0.62, phoneMode ? "rgba(4,7,7,0.34)" : "rgba(3,5,5,0.58)");
    settle.addColorStop(1, phoneMode ? "rgba(0,0,0,0.56)" : "rgba(0,0,0,0.72)");
  }
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    ctx.globalAlpha = phoneMode ? 0.16 : 0.2;
    ctx.beginPath();
    for (let step = 0; step <= 18; step += 1) {
      const t = step / 18;
      const y = horizon + (h - horizon + 18) * t;
      const x = perspectiveRoadCenter(w, t) + side * (realWorldRoadHalf(roadTop, roadBottom, t) * 0.98);
      if (step === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
    ctx.stroke();
...
function roadsideAnchorX(w, h, horizon, roadTop, roadBottom, y, side, projectedX, phoneMode, extra = 0) {
  ...
  const clearance = extra + w * (phoneMode ? 0.075 : 0.052) + t * w * (phoneMode ? 0.12 : 0.086);
...
function drawRealWorldRoadSurface(w, h, horizon, roadTop, roadBottom, place, design, theme, seed) {
  ...
  const baseRoad = ctx.createLinearGradient(0, horizon, 0, h);
  if (place === "desert" || place === "canyon" || place === "farm") {
    baseRoad.addColorStop(0, phoneMode ? "rgba(52,47,36,0.94)" : "rgba(54,50,38,0.88)");
    baseRoad.addColorStop(0.45, phoneMode ? "rgba(28,25,20,0.97)" : "rgba(30,27,22,0.92)");
  } else if (place === "snow" || place === "alpine" || place === "iceland") {
    baseRoad.addColorStop(0, phoneMode ? "rgba(44,54,54,0.94)" : "rgba(48,58,58,0.88)");
    baseRoad.addColorStop(0.45, phoneMode ? "rgba(20,26,27,0.97)" : "rgba(24,30,31,0.92)");
  } else if (place === "coast" || place === "harbor" || place === "rainforest" || place === "venice" || place === "monaco" || place === "dubai" || place === "rio") {
    baseRoad.addColorStop(0, phoneMode ? "rgba(28,39,39,0.95)" : "rgba(30,42,42,0.9)");
    baseRoad.addColorStop(0.45, phoneMode ? "rgba(12,18,18,0.98)" : "rgba(14,20,20,0.94)");
  } else {
    baseRoad.addColorStop(0, phoneMode ? "rgba(31,36,35,0.95)" : "rgba(34,39,38,0.9)");
    baseRoad.addColorStop(0.45, phoneMode ? "rgba(13,17,16,0.98)" : "rgba(16,20,19,0.94)");
  }
  baseRoad.addColorStop(1, phoneMode ? "rgba(2,3,3,0.99)" : "rgba(3,5,5,0.96)");
...
function drawRealWorldWeatherDisasterPass(w, h, horizon, roadTop, roadBottom, place, route, design, theme, seed) {
  const phoneMode = phoneGraphicsActive();
  const length = Math.max(1, raceLength());
  const progress = Math.max(0, Math.min(1, raceState.distance / length));
  const condition = raceState.roadCondition || routeWeatherCondition(place, progress, raceState.elapsed || 0);
  const motion = (raceState.roadOffset || 0) * (0.62 + Math.min(1.1, Math.max(0, raceState.speed || 0) / 260));
  ctx.save();
  drawWeatherSkyMoodPass(w, h, horizon, place, condition, design, theme, seed, motion, phoneMode);
  drawWeatherRoadConditionSurface(w, h, horizon, roadTop, roadBottom, place, condition, design, theme, seed, motion, phoneMode);
  drawWeatherRailAndHazardPass(w, h, horizon, roadTop, roadBottom, place, route, condition, design, theme, seed, motion, phoneMode);
  ctx.restore();
}

function drawWeatherSkyMoodPass(w, h, horizon, place, condition, design, theme, seed, motion, phoneMode) {
  const sky = condition.sky || "sun";
  ctx.save();
  if (sky === "night") {
    const night = ctx.createLinearGradient(0, 0, 0, horizon + h * 0.18);
    night.addColorStop(0, "rgba(2,7,16,0.34)");
    night.addColorStop(1, "rgba(2,7,16,0)");
    ctx.fillStyle = night;
    ctx.fillRect(0, 0, w, horizon + h * 0.2);
    ctx.globalCompositeOperation = "screen";
    ctx.fillStyle = `${design.light || theme[2] || "#ffd166"}44`;
    for (let i = 0; i < (phoneMode ? 5 : 8); i += 1) {
      const x = w * (0.08 + seededUnit(seed, i + 90) * 0.84);
      const y = horizon * (0.18 + seededUnit(seed, i + 95) * 0.62);
      ctx.beginPath();
      ctx.arc(x, y, 1.2 + seededUnit(seed, i + 96) * 1.8, 0, Math.PI * 2);
      ctx.fill();
    }
  } else if (sky === "sun") {
    const x = w * (0.18 + seededUnit(seed, 102) * 0.18);
    const y = horizon * (0.2 + seededUnit(seed, 103) * 0.22);
    const r = Math.max(24, Math.min(58, w * 0.08));
    ctx.globalCompositeOperation = "screen";
    const sun = ctx.createRadialGradient(x, y, 2, x, y, r * 2.6);
    sun.addColorStop(0, "rgba(255,245,194,0.68)");
    sun.addColorStop(0.36, "rgba(255,209,102,0.28)");
    sun.addColorStop(1, "rgba(255,209,102,0)");
    ctx.fillStyle = sun;
    ctx.beginPath();
    ctx.arc(x, y, r * 2.6, 0, Math.PI * 2);
    ctx.fill();
  } else if (sky === "cloud" || sky === "rain" || sky === "snow" || sky === "storm") {
    const cloudAlpha = sky === "storm" ? 0.34 : sky === "cloud" ? 0.22 : 0.18;
    ctx.fillStyle = `rgba(14,24,31,${cloudAlpha})`;
    for (let i = 0; i < (phoneMode ? 5 : 8); i += 1) {
      const x = w * (0.04 + seededUnit(seed, i + 120) * 0.92) + Math.sin(motion * 0.004 + i) * 24;
      const y = horizon * (0.16 + seededUnit(seed, i + 130) * 0.48);
      const sx = w * (0.11 + seededUnit(seed, i + 140) * 0.12);
      const sy = h * (0.018 + seededUnit(seed, i + 150) * 0.026);
      ctx.beginPath();
      ctx.ellipse(x, y, sx, sy, 0, 0, Math.PI * 2);
      ctx.fill();
    }
    if (sky === "storm") {
      ctx.strokeStyle = "rgba(255,245,194,0.28)";
      ctx.lineWidth = phoneMode ? 1.4 : 2;
      const lx = w * (0.64 + seededUnit(seed, 161) * 0.2);
      ctx.beginPath();
      ctx.moveTo(lx, horizon * 0.22);
      ctx.lineTo(lx - w * 0.025, horizon * 0.38);
      ctx.lineTo(lx + w * 0.018, horizon * 0.36);
      ctx.lineTo(lx - w * 0.018, horizon * 0.54);
      ctx.stroke();
    }
  }

  if (sky === "rain" || sky === "storm") {
    ctx.strokeStyle = sky === "storm" ? "rgba(222,249,255,0.32)" : "rgba(222,249,255,0.22)";
    ctx.lineWidth = phoneMode ? 1 : 1.4;
    const drops = phoneMode ? 26 : 44;
    for (let i = 0; i < drops; i += 1) {
      const x = (seededUnit(seed, i + 180) * w + motion * 0.08) % w;
      const y = horizon * 0.12 + seededUnit(seed, i + 190) * h * 0.74;
      const len = phoneMode ? 7 + seededUnit(seed, i + 200) * 9 : 10 + seededUnit(seed, i + 200) * 16;
      ctx.beginPath();
      ctx.moveTo(x, y);
    ...
    const flakes = phoneMode ? 28 : 48;
    for (let i = 0; i < flakes; i += 1) {
      const x = (seededUnit(seed, i + 210) * w + Math.sin(motion * 0.006 + i) * 18) % w;
      const y = (seededUnit(seed, i + 220) * h + motion * 0.12) % h;
      ctx.beginPath();
      ctx.arc(x, y, 1.1 + seededUnit(seed, i + 230) * 2.2, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.restore();
...
function drawWeatherRoadConditionSurface(w, h, horizon, roadTop, roadBottom, place, condition, design, theme, seed, motion, phoneMode) {
  const surface = condition.surface || "dry";
  if (surface === "dry") return;
  const count = phoneMode ? 7 : 12;
  ctx.save();
  clipRealWorldRoad(w, h, horizon, roadTop, roadBottom);
  if (surface === "standingWater" || surface === "wet") {
    ctx.globalCompositeOperation = "screen";
    const wash = ctx.createLinearGradient(0, horizon, 0, h);
    wash.addColorStop(0, "rgba(70,217,255,0)");
    wash.addColorStop(0.66, surface === "standingWater" ? "rgba(103,216,255,0.16)" : "rgba(222,249,255,0.08)");
    wash.addColorStop(1, surface === "standingWater" ? "rgba(103,216,255,0.22)" : "rgba(222,249,255,0.1)");
    ctx.fillStyle = wash;
    ctx.fillRect(0, horizon, w, h - horizon);
    ctx.globalCompositeOperation = "source-over";
  }
  for (let i = 0; i < count; i += 1) {
    const y = horizon + (((i * 151 + motion * 0.82 + seed * 0.004) % (h - horizon + 240)) - 70);
    if (y < horizon + 4 || y > h + 34) continue;
    const t = Math.max(0.07, Math.min(1.06, (y - horizon) / Math.max(1, h - horizon)));
    const lane = -1.45 + seededUnit(seed, i + 260) * 2.9;
    const x = realWorldRoadX(w, lane, t);
    const width = (38 + seededUnit(seed, i + 270) * 102) * (0.38 + t * 0.84);
    const height = (5 + seededUnit(seed, i + 280) * 22) * (0.42 + t * 0.9);
    drawWeatherRoadPatch(surface, x, y, width, height, t, place, design, theme, seed + i * 17);
  }
  if (condition.hazard === "earthquake" || surface === "cracked") {
    drawWeatherQuakeRoadCracks(w, h, horizon, seed, motion, phoneMode);
  }
  ctx.restore();
}

function drawWeatherRoadPatch(surface, x, y, width, height, t, place, design, theme, seed) {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate((seededUnit(seed, 1) - 0.5) * 0.12);
  if (surface === "wet" || surface === "standingWater") {
    ctx.globalCompositeOperation = "screen";
    const puddle = ctx.createRadialGradient(0, 0, 1, 0, 0, Math.max(width, height));
    puddle.addColorStop(0, surface === "standingWater" ? "rgba(147,232,255,0.38)" : "rgba(222,249,255,0.2)");
    puddle.addColorStop(0.48, surface === "standingWater" ? "rgba(70,217,255,0.2)" : "rgba(70,217,255,0.1)");
    ...
    ctx.quadraticCurveTo(0, -height * 0.7, width * 0.44, -height * 0.05);
    ctx.stroke();
  } else if (surface === "oilSlick" || surface === "blackIce") {
    ctx.globalCompositeOperation = "screen";
    const gloss = ctx.createRadialGradient(0, 0, 1, 0, 0, Math.max(width, height));
    gloss.addColorStop(0, surface === "blackIce" ? "rgba(8,18,28,0.78)" : "rgba(0,0,0,0.7)");
    gloss.addColorStop(0.58, surface === "blackIce" ? "rgba(70,217,255,0.18)" : `${design.accent || theme[1] || "#46d9ff"}24`);
    gloss.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = gloss;
    ctx.beginPath();
    ctx.ellipse(0, 0, width * 0.82, height, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = surface === "blackIce" ? "rgba(222,249,255,0.36)" : "rgba(255,209,102,0.22)";
    ctx.lineWidth = Math.max(1, height * 0.1);
    ctx.beginPath();
    ctx.moveTo(-width * 0.3, -height * 0.35);
    ctx.lineTo(width * 0.32, -height * 0.18);
    ctx.stroke();
  } else if (surface === "snowSlush") {
    ctx.fillStyle = "rgba(244,251,248,0.36)";
    ctx.beginPath();
    ctx.ellipse(0, 0, width * 0.75, height * 0.92, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "rgba(70,217,255,0.16)";
    ctx.beginPath();
    ctx.ellipse(width * 0.18, height * 0.1, width * 0.34, height * 0.48, 0, 0, Math.PI * 2);
    ctx.fill();
  } else if (surface === "mud" || surface === "dust") {
    ctx.fillStyle = surface === "mud" ? "rgba(72,42,22,0.42)" : "rgba(255,183,74,0.26)";
    ctx.beginPath();
    ctx.ellipse(0, 0, width * 0.68, height * 0.82, 0, 0, Math.PI * 2);
    ctx.fill();
  } else if (surface === "cracked") {
    ctx.strokeStyle = "rgba(0,0,0,0.5)";
    ctx.lineWidth = Math.max(1, 1.4 + t * 2.8);
    ctx.beginPath();
    ctx.moveTo(-width * 0.42, 0);
    ctx.lineTo(-width * 0.14, -height * 0.6);
    ctx.lineTo(width * 0.1, height * 0.32);
    ctx.lineTo(width * 0.44, -height * 0.2);
    ctx.stroke();
  }
  ctx.restore();
}

function drawWeatherQuakeRoadCracks(w, h, horizon, seed, motion, phoneMode) {
  ctx.save();
  ctx.strokeStyle = "rgba(0,0,0,0.56)";
  ctx.lineCap = "round";
  const cracks = phoneMode ? 4 : 7;
  for (let i = 0; i < cracks; i += 1) {
    const y = horizon + (((i * 211 + motion * 0.52 + seed * 0.005) % (h - horizon + 260)) - 82);
    if (y < horizon + 10 || y > h * 0.96) continue;
    const t = Math.max(0.08, Math.min(1.05, (y - horizon) / Math.max(1, h - horizon)));
    const x = realWorldRoadX(w, -0.9 + seededUnit(seed, i + 310) * 1.8, t);
    ctx.lineWidth = Math.max(1.5, 1.8 + t * 4.2);
    ctx.beginPath();
    ctx.moveTo(x - 38 * t, y);
    ctx.lineTo(x - 14 * t, y + 9 * t);
    ctx.lineTo(x + 8 * t, y - 5 * t);
    ctx.lineTo(x + 42 * t, y + 12 * t);
    ctx.stroke();
    ctx.strokeStyle = "rgba(244,251,248,0.12)";
    ctx.beginPath();
    ctx.moveTo(x - 20 * t, y + 5 * t);
    ctx.lineTo(x + 34 * t, y + 16 * t);
    ctx.stroke();
    ctx.strokeStyle = "rgba(0,0,0,0.56)";
  }
  ctx.restore();
...
function drawWeatherRailAndHazardPass(w, h, horizon, roadTop, roadBottom, place, route, condition, design, theme, seed, motion, phoneMode) {
  ctx.save();
  clipOutsideRealWorldRoad(w, h, horizon, roadTop, roadBottom, phoneMode ? 0 : 4);
  if (condition.hazard === "train" || place === "freight" || place === "city" || place === "tokyo" || place === "europe") {
    drawWeatherPassingTrain(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, motion, phoneMode);
  }
  if (condition.hazard === "tornado") drawWeatherTornado(w, h, horizon, roadTop, roadBottom, design, theme, seed, motion, phoneMode);
  if (condition.hazard === "tidalSurge" || condition.hazard === "coastalFlood" || condition.hazard === "floodedBridge") {
    drawWeatherCoastalFloodView(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, motion, phoneMode);
  }
  if (condition.hazard === "earthquake") drawWeatherEarthquakeRoadsideDamage(w, h, horizon, roadTop, roadBottom, design, theme, seed, motion, phoneMode);
  ctx.restore();
}

function drawWeatherPassingTrain(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, motion, phoneMode) {
  const side = seededUnit(seed, 351) > 0.48 ? 1 : -1;
  const t = phoneMode ? 0.26 + seededUnit(seed, 352) * 0.16 : 0.22 + seededUnit(seed, 352) * 0.18;
  const y = horizon + (h - horizon) * t + Math.sin(motion * 0.004) * 6;
  const half = realWorldRoadHalf(roadTop, roadBottom, t);
  const baseX = perspectiveRoadCenter(w, t) + side * (half + w * (phoneMode ? 0.17 : 0.2));
  const trainMotion = ((motion * 0.7 + seed * 0.02) % (w * 0.42)) - w * 0.21;
  const x = baseX + side * trainMotion;
  const scale = 0.62 + t * (phoneMode ? 1.0 : 1.15);
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(scale * side, scale);
  ctx.strokeStyle = "rgba(180,192,188,0.7)";
  ...
  ctx.beginPath();
  ctx.ellipse(0, 43, 170, 13, -0.03, 0, Math.PI * 2);
  ctx.fill();
  const carColor = place === "tokyo" ? `${design.accent || theme[1] || "#46d9ff"}cc` : place === "freight" ? "rgba(255,183,74,0.82)" : "rgba(220,232,239,0.82)";
  for (let i = -2; i <= 2; i += 1) {
    const cx = i * 58;
    ctx.fillStyle = carColor;
    roundRect(cx - 26, -14 + (i % 2) * 2, 52, 31, 5);
    ctx.fill();
    ctx.fillStyle = "rgba(5,8,7,0.72)";
    roundRect(cx - 17, -8, 34, 8, 2);
    ctx.fill();
    ctx.fillStyle = "#050807";
    ctx.beginPath();
    ctx.arc(cx - 15, 19, 5, 0, Math.PI * 2);
    ctx.arc(cx + 15, 19, 5, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
...
function drawWeatherTornado(w, h, horizon, roadTop, roadBottom, design, theme, seed, motion, phoneMode) {
  const side = seededUnit(seed, 380) > 0.5 ? 1 : -1;
  const x = w * (side > 0 ? 0.78 : 0.22) + Math.sin(motion * 0.003) * 18;
  const baseY = horizon + h * (phoneMode ? 0.22 : 0.2);
  const topY = horizon * 0.36;
  ctx.save();
  ctx.globalAlpha = phoneMode ? 0.48 : 0.42;
  ctx.strokeStyle = "rgba(60,62,58,0.72)";
  ctx.lineWidth = phoneMode ? 8 : 11;
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(x - side * 32, topY);
  ctx.bezierCurveTo(x + side * 26, topY + 54, x - side * 20, baseY - 42, x + side * 10, baseY);
  ctx.stroke();
  ctx.strokeStyle = "rgba(244,251,248,0.18)";
  ctx.lineWidth = phoneMode ? 3 : 4;
  for (let i = 0; i < 4; i += 1) {
    const y = topY + i * ((baseY - topY) / 4);
    ctx.beginPath();
    ctx.ellipse(x + Math.sin(motion * 0.008 + i) * 12, y, 42 - i * 7, 7, -0.15, 0, Math.PI * 2);
    ctx.stroke();
  }
  ctx.fillStyle = "rgba(117,86,47,0.26)";
  ctx.beginPath();
  ctx.ellipse(x + side * 8, baseY + 10, 72, 14, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}

function drawWeatherCoastalFloodView(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, motion, phoneMode) {
  const side = place === "harbor" || seededUnit(seed, 410) > 0.5 ? 1 : -1;
  ctx.save();
  ctx.globalCompositeOperation = "screen";
  for (let i = 0; i < (phoneMode ? 4 : 6); i += 1) {
    const t = 0.36 + i * 0.11;
    const y = horizon + (h - horizon) * t + Math.sin(motion * 0.01 + i) * 7;
    const half = realWorldRoadHalf(roadTop, roadBottom, t);
    const x = perspectiveRoadCenter(w, t) + side * (half + w * (0.05 + t * 0.08));
    const width = w * (0.22 + t * 0.22);
    const height = h * (0.018 + t * 0.025);
    const wave = ctx.createRadialGradient(x, y, 1, x, y, width);
    wave.addColorStop(0, "rgba(147,232,255,0.46)");
    wave.addColorStop(0.48, "rgba(70,217,255,0.18)");
    wave.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = wave;
    ctx.beginPath();
    ctx.ellipse(x, y, width, height, 0.02 * side, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(244,251,248,0.38)";
    ctx.lineWidth = Math.max(1.4, 2 + t * 4);
    ctx.beginPath();
    ctx.moveTo(x - side * width * 0.54, y - height * 0.2);
    ctx.quadraticCurveTo(x, y - height * 1.3, x + side * width * 0.48, y - height * 0.1);
    ctx.stroke();
  }
  ctx.restore();
}

function drawWeatherEarthquakeRoadsideDamage(w, h, horizon, roadTop, roadBottom, design, theme, seed, motion, phoneMode) {
  ctx.save();
  ctx.globalAlpha = phoneMode ? 0.62 : 0.5;
  for (let i = 0; i < (phoneMode ? 4 : 6); i += 1) {
    const side = i % 2 === 0 ? -1 : 1;
    const t = 0.34 + seededUnit(seed, i + 440) * 0.44;
    const y = horizon + (h - horizon) * t;
    const half = realWorldRoadHalf(roadTop, roadBottom, t);
    const x = perspectiveRoadCenter(w, t) + side * (half + w * (0.04 + t * 0.06));
    const scale = 0.6 + t * 1.0;
    ctx.save();
    ctx.translate(x, y);
    ctx.scale(scale * side, scale);
    ctx.fillStyle = "rgba(72,72,68,0.72)";
    roundRect(-42, -8, 84, 16, 3);
    ctx.fill();
    ...
    ctx.lineTo(30, 8);
    ctx.stroke();
    ctx.fillStyle = `${design.light || theme[2] || "#ffd166"}66`;
    ctx.fillRect(-38, -18, 14, 7);
    ctx.restore();
  }
  ctx.restore();
}
...
function drawRealWorldRoadHardware(w, h, horizon, roadTop, roadBottom, place, design, theme, seed, motion) {
  ...
  for (let i = 0; i < hardwareCount; i += 1) {
    ...
    if (kind === "manhole") {
      ...
      for (let s = phoneMode ? -1 : -3; s <= (phoneMode ? 1 : 3); s += 1) {
        ctx.beginPath();
      ...
      ctx.globalCompositeOperation = "screen";
      ctx.fillStyle = "rgba(222,249,255,0.16)";
      ctx.beginPath();
      ctx.ellipse(0, 0, size * 1.45, size * 0.18, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = "rgba(244,251,248,0.2)";
      ctx.lineWidth = Math.max(1, size * 0.05);
      ctx.beginPath();
      ctx.moveTo(-size * 1.2, 0);
      ctx.lineTo(size * 1.2, size * 0.08);
      ctx.stroke();
    } else {
      ctx.fillStyle = "rgba(244,251,248,0.42)";
      ctx.beginPath();
  ...
  const shoulderLines = phoneMode ? 2 : 6;
  for (let side = -1; side <= 1; side += 2) {
...
function drawRoadsideFence(side, color) {
  if (phoneGraphicsActive()) {
    ctx.fillStyle = color && color.includes("244") ? "rgba(230,240,236,0.5)" : "rgba(70,112,72,0.45)";
    ctx.beginPath();
    ctx.ellipse(4 * side, -5, 76, 18, -0.05 * side, 0, Math.PI * 2);
    ctx.ellipse(-38 * side, -15, 38, 14, 0.08 * side, 0, Math.PI * 2);
    ctx.ellipse(44 * side, -16, 42, 15, -0.08 * side, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = color;
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(-68 * side, -18);
    ctx.lineTo(72 * side, -24);
    ctx.moveTo(-68 * side, 0);
    ctx.lineTo(72 * side, -6);
    ctx.stroke();
    return;
  }
  ctx.strokeStyle = color;
...
function drawRealWorldHorizonDetails(w, h, horizon, place, design, theme, seed) {
  ...
  if (place === "alpine" || place === "snow" || place === "europe" || place === "canyon" || place === "desert" || place === "iceland") {
    for (let layer = 0; layer < 2; layer += 1) {
      ctx.fillStyle = layer === 0
        ? (place === "iceland" ? "rgba(130,174,180,0.42)" : "rgba(54,72,74,0.36)")
        : (place === "desert" || place === "canyon" ? "rgba(132,72,38,0.34)" : "rgba(150,174,174,0.3)");
      ctx.beginPath();
  ...
  } else if (place === "city" || place === "tokyo" || place === "vegas" || place === "dubai" || place === "monaco") {
    for (let i = 0; i < 18; i += 1) {
      const x = ((i * w * 0.072 - offset * 7) % (w + 140)) - 70;
      const bw = w * (0.028 + seededUnit(seed, i) * 0.032);
      const extraHeight = place === "dubai" ? h * 0.07 : place === "vegas" || place === "monaco" ? h * 0.035 : 0;
      const bh = h * (0.08 + seededUnit(seed, i + 8) * 0.16) + extraHeight;
      ctx.fillStyle = place === "tokyo" || place === "vegas" ? "rgba(20,10,38,0.72)"
        : place === "dubai" ? "rgba(10,46,54,0.74)"
          : place === "monaco" ? "rgba(36,34,34,0.7)"
            : "rgba(8,18,21,0.66)";
      ...
      if ((place === "vegas" || place === "monaco") && i % 5 === 0) {
        ctx.fillStyle = `${design.light}55`;
        roundRect(x - bw * 0.12, base - bh - 8, bw * 1.24, 7, 3);
        ctx.fill();
      }
    }
  } else if (place === "coast" || place === "harbor" || place === "venice" || place === "rio") {
    const water = ctx.createLinearGradient(0, horizon + h * 0.02, 0, horizon + h * 0.18);
    water.addColorStop(0, place === "rio" ? "rgba(54,217,138,0.12)" : "rgba(70,217,255,0.18)");
    water.addColorStop(1, "rgba(4,20,24,0.38)");
    ctx.fillStyle = water;
    ctx.fillRect(0, horizon + h * 0.015, w, h * 0.16);
    if (place === "rio") {
      ctx.fillStyle = "rgba(28,102,58,0.4)";
      for (let i = 0; i < 7; i += 1) {
        const x = ((i * w * 0.16 - offset * 8) % (w + 160)) - 80;
        ctx.beginPath();
        ctx.moveTo(x - w * 0.08, horizon + h * 0.14);
        ctx.quadraticCurveTo(x + w * 0.02, horizon - h * (0.03 + seededUnit(seed, i) * 0.09), x + w * 0.16, horizon + h * 0.14);
        ctx.closePath();
        ctx.fill();
      }
    }
    ...
    if (place === "venice") {
      ctx.strokeStyle = "rgba(190,178,150,0.34)";
      ctx.lineWidth = Math.max(2, w * 0.004);
      for (let i = 0; i < 4; i += 1) {
        const x = ((i * w * 0.26 - offset * 9) % (w + 180)) - 90;
        ctx.beginPath();
        ctx.moveTo(x, horizon + h * 0.12);
        ctx.quadraticCurveTo(x + w * 0.08, horizon + h * 0.02, x + w * 0.18, horizon + h * 0.12);
        ctx.stroke();
      }
    }
...
function drawRealWorldRouteSignaturePass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed) {
  ...
  const fixtures = {
    ...
    vegas: ["hotelMarquee", "neonBlocks", "expressGantry"],
    dubai: ["glassMarinaTowers", "pierRails", "drawBridge"],
    venice: ["canalBridge", "stoneVillage", "pierRails"],
    iceland: ["iceWall", "stoneTunnel", "roadCut"],
    monaco: ["cliffVillas", "pierRails", "alpsTunnel"],
    rio: ["hillsideHomes", "beachHouse", "canopyBridge"]
  };
  ...
  if ((!phoneMode || h > 620) && !routePrefersLowSignage(place)) {
    const signT = 0.22;
...
function drawRouteSignatureFixture(kind, x, y, scale, side, place, design, theme, seed) {
  ...
  if (kind === "overpass" || kind === "expressGantry" || kind === "interstateGantry" || kind === "railOverpass") {
  ...
  } else if (kind === "hotelMarquee" || kind === "glassMarinaTowers" || kind === "cliffVillas" || kind === "hillsideHomes") {
    if (kind === "hillsideHomes") {
      ctx.fillStyle = "rgba(34,102,58,0.44)";
      ctx.beginPath();
      ctx.moveTo(-96, 18);
      ctx.quadraticCurveTo(-28, -86, 106, 14);
      ctx.closePath();
      ctx.fill();
      for (let row = 0; row < 3; row += 1) {
        for (let col = -2; col <= 2; col += 1) {
          const px = col * 26 + row * 9 * side;
          const py = -10 - row * 24 - Math.abs(col) * 3;
          ctx.fillStyle = row % 2 ? "rgba(238,216,172,0.88)" : "rgba(210,232,220,0.84)";
          roundRect(px - 11, py - 17, 22, 17, 3);
          ctx.fill();
          ctx.fillStyle = "rgba(136,48,40,0.86)";
          ctx.beginPath();
    ...
    } else if (kind === "cliffVillas") {
      ctx.fillStyle = "rgba(120,130,116,0.48)";
      ctx.beginPath();
      ctx.moveTo(-104, 20);
      ctx.lineTo(-48, -76);
      ctx.lineTo(22, -48);
      ...
      ctx.fill();
      for (let i = -1; i <= 1; i += 1) {
        ctx.fillStyle = "rgba(238,232,208,0.9)";
        roundRect(i * 44 - 22, -74 - Math.abs(i) * 10, 44, 32, 4);
        ctx.fill();
        ctx.fillStyle = `${light}88`;
        ctx.beginPath();
        ctx.moveTo(i * 44 - 26, -74 - Math.abs(i) * 10);
        ctx.lineTo(i * 44, -98 - Math.abs(i) * 10);
        ctx.lineTo(i * 44 + 26, -74 - Math.abs(i) * 10);
        ctx.closePath();
        ctx.fill();
      }
    } else {
      const glass = kind === "glassMarinaTowers";
      const count = glass ? 5 : 4;
      for (let i = 0; i < count; i += 1) {
        const bw = glass ? 30 + i % 2 * 10 : 44;
        const bh = (glass ? 108 : 82) + seededUnit(seed, i + 12) * (glass ? 64 : 36);
        const px = -78 + i * (glass ? 38 : 52);
        const tower = ctx.createLinearGradient(px, -bh, px + bw, 0);
        tower.addColorStop(0, glass ? "rgba(180,236,238,0.46)" : "rgba(255,79,216,0.22)");
        tower.addColorStop(0.42, glass ? "rgba(18,58,66,0.86)" : "rgba(24,12,40,0.9)");
        tower.addColorStop(1, "rgba(5,8,7,0.92)");
        ctx.fillStyle = tower;
        roundRect(px, -bh, bw, bh, 5);
        ctx.fill();
        ctx.fillStyle = i % 2 ? `${accent}77` : `${light}66`;
        for (let row = 0; row < 5; row += 1) roundRect(px + bw * 0.22, -bh + 16 + row * (bh / 6), bw * 0.56, 4, 2);
        ctx.fill();
      }
      if (glass) {
        ctx.fillStyle = "rgba(28,80,90,0.36)";
        ctx.beginPath();
        ctx.ellipse(12, 20, 112, 20, 0, 0, Math.PI * 2);
        ctx.fill();
      } else {
        ctx.fillStyle = `${light}88`;
        roundRect(-88, -142, 176, 18, 5);
        ctx.fill();
      }
    }
    ...
    ctx.stroke();
  } else if (kind === "canalBridge" || kind === "iceWall") {
    if (kind === "canalBridge") {
      ctx.strokeStyle = "rgba(190,178,150,0.84)";
      ctx.lineWidth = 7;
      ctx.beginPath();
      ctx.moveTo(-94, -8);
      ctx.quadraticCurveTo(0, -76, 94, -8);
      ctx.stroke();
      ctx.fillStyle = "rgba(20,64,72,0.34)";
      ctx.beginPath();
      ctx.ellipse(0, 12, 104, 18, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillStyle = "rgba(178,224,226,0.58)";
      ctx.beginPath();
      ctx.moveTo(-106, 20);
      ctx.lineTo(-70, -62);
      ctx.lineTo(-28, -36);
...
function livingWorldActivitySet(place) {
  const sets = {
    ...
    vegas: ["neonCrowd", "sidewalkQueue", "cyclistLane", "cameraCrew", "deliveryVan", "patioCrowd"],
    dubai: ["pierPeople", "dockWorkers", "boatWake", "sidewalkQueue", "cameraCrew", "serviceCartMotion"],
    venice: ["pierPeople", "dockWorkers", "boatWake", "patioCrowd", "villageWalkers", "shoreBirds"],
    iceland: ["hikerLookout", "wildlifeLookout", "snowMarkerCrew", "rallyCrew", "snowPlowCrew", "cabinPorch"],
    monaco: ["patioCrowd", "cyclistLane", "pierPeople", "cameraCrew", "villageWalkers", "boatWake"],
    rio: ["beachJoggers", "cyclistLane", "marketLife", "canopyAnimals", "patioCrowd", "hikerLookout"]
  };
  return sets[place] || sets.city;
}
...
function drawRealWorldDistrictMotionPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed) {
  ...
  const count = phoneMode ? 7 : 12;
  ctx.save();
  ctx.globalAlpha = phoneMode ? 0.42 : 0.34;
  for (let i = 0; i < count; i += 1) {
    const side = i % 2 === 0 ? -1 : 1;
...
function drawDistrictMotionShape(kind, x, y, scale, side, place, design, theme, seed) {
  ...
  if (kind === "sideRoadCar" || kind === "deliveryVan" || kind === "tourBus" || kind === "farmTruck" || kind === "sideRoadTruck" || kind === "lookoutCar" || kind === "dustTruck" || kind === "snowPlow" || kind === "villageCar") {
    ...
    ctx.strokeStyle = phoneMode ? "rgba(180,192,188,0.74)" : "rgba(180,192,188,0.5)";
    ctx.lineWidth = phoneMode ? 4 : 3;
    ctx.beginPath();
    ...
    ctx.stroke();
    ctx.fillStyle = kind === "railCar" ? "rgba(255,209,102,0.78)" : `${accent}86`;
    const cars = phoneMode ? 3 : 5;
    for (let i = -Math.floor(cars / 2); i <= Math.floor(cars / 2); i += 1) {
      const carW = phoneMode ? 42 : 26;
      roundRect(i * (phoneMode ? 47 : 30) - carW / 2, -18 + (i % 2) * 2, carW, 22, 3);
      ctx.fill();
      if (phoneMode) {
        ctx.fillStyle = "rgba(5,8,7,0.58)";
        roundRect(i * 47 - 14, -12, 28, 6, 2);
        ctx.fill();
        ctx.fillStyle = kind === "railCar" ? "rgba(255,209,102,0.78)" : `${accent}86`;
      }
    }
...
function realWorldRoadRuleKind(place, index, seed) {
  const sets = {
    ...
    vegas: ["turnOnly", "merge", "stopBar", "tunnelBands"],
    dubai: ["merge", "bridgeJoint", "turnOnly", "crosswalk"],
    venice: ["dockLane", "bridgeJoint", "slowCurve", "wetBridge"],
    iceland: ["iceWarning", "snowEdge", "slowCurve", "bridgeJoint"],
    monaco: ["turnOnly", "slowCurve", "crosswalk", "tunnelBands"],
    rio: ["slowCurve", "bikeLane", "villageCrossing", "wetBridge"]
  };
  const list = sets[place] || sets.city;
  return list[(index + Math.floor(seed % list.length)) % list.length];
}
...
function drawRoadRuleMark(type, w, horizon, y, t, place, design, theme, seed) {
  const phoneMode = phoneGraphicsActive();
  const alpha = phoneMode ? 0.045 + t * 0.028 : 0.18 + t * 0.08;
  ctx.save();
  ctx.globalAlpha = alpha;
  const lane = type === "busLane" || type === "bikeLane" || type === "truckLane" ? -1.15 : type === "merge" ? 1.1 : 0;
  const x = realWorldRoadX(w, lane, t);
  if (phoneMode && type === "merge") {
    drawPerspectivePavementQuad(w, horizon, 0.9, y, 4 + t * 5, laneWidth() * 0.78, "rgba(244,251,248,0.34)");
    ctx.restore();
    return;
  }
  if (type === "bridgeJoint" || type === "tunnelBands" || type === "runwayBars") {
    const stripes = phoneMode ? (type === "runwayBars" ? 4 : 2) : (type === "runwayBars" ? 6 : 4);
    ...
    const bars = phoneMode ? (type === "crosswalk" || type === "villageCrossing" ? 3 : 1) : (type === "crosswalk" || type === "villageCrossing" ? 6 : 2);
    ...
    ctx.scale(0.7 + t * 0.9, 0.36 + t * 0.42);
      ctx.fillStyle = phoneMode ? "rgba(244,251,248,0.54)" : type === "busLane" || type === "bikeLane" ? `${design.accent}66` : type === "iceWarning" ? "rgba(244,251,248,0.68)" : type === "truckLane" || type === "weighStation" ? "rgba(255,209,102,0.7)" : "rgba(244,251,248,0.62)";
    if (type === "merge") {
      ctx.beginPath();
      ctx.moveTo(-36, 32);
      ctx.lineTo(-4, -32);
...
function realWorldShadowKind(place, index, seed) {
  const sets = {
    ...
    vegas: ["underpass", "lightPool", "wireShadow"],
    dubai: ["underpass", "lightPool", "wireShadow"],
    monaco: ["underpass", "lightPool", "wireShadow"],
    rio: ["treeShadow", "wireShadow", "lightPool"],
    venice: ["underpass", "lightPool", "wireShadow"],
    iceland: ["treeShadow", "snowLight", "underpass"],
...
function drawRealWorldEdgeMaterialPass(w, h, horizon, roadTop, roadBottom, place, design, theme, seed) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (place === "snow" || place === "alpine" || place === "iceland") {
      ctx.fillStyle = "rgba(244,251,248,0.58)";
      ctx.beginPath();
      ctx.ellipse(x, y, size * 1.7, size * 0.38, 0, 0, Math.PI * 2);
      ctx.fill();
    } else if (place === "desert" || place === "canyon" || place === "farm" || place === "vegas") {
      ...
      ctx.stroke();
    } else if (place === "rainforest" || place === "city" || place === "tokyo" || place === "europe" || place === "rio" || place === "dubai" || place === "monaco") {
      ctx.fillStyle = place === "rainforest" || place === "rio" ? "rgba(54,217,138,0.24)" : "rgba(244,251,248,0.16)";
      roundRect(x - size * 1.2, y - size * 0.12, size * 2.4, size * 0.25, Math.max(2, size * 0.08));
      ctx.fill();
      ctx.fillStyle = place === "rainforest" || place === "rio" ? "rgba(36,110,62,0.36)" : "rgba(0,0,0,0.28)";
      ctx.beginPath();
...
function drawGenAiHeroBackdrop(w, h, horizon, design, theme, place, route, seed) {
  ...
  if (place === "city" || place === "tokyo" || place === "vegas" || place === "dubai" || place === "monaco") {
    for (let i = 0; i < 18; i += 1) {
      const bw = w * (0.025 + seededUnit(seed, i) * 0.035);
      const extraHeight = place === "dubai" ? 0.12 : place === "vegas" || place === "monaco" ? 0.06 : 0;
      const bh = h * (0.16 + extraHeight + seededUnit(seed, i + 11) * 0.22);
      const x = ((i * w * 0.078 - (raceState.roadOffset || 0) * 0.018) % (w + 140)) - 70;
      const tower = ctx.createLinearGradient(x, farBase - bh, x + bw, farBase);
      tower.addColorStop(0, place === "tokyo" || place === "vegas" ? "rgba(255,79,216,0.24)" : place === "dubai" ? "rgba(170,230,235,0.24)" : "rgba(255,209,102,0.18)");
      tower.addColorStop(0.32, place === "dubai" ? "rgba(20,60,68,0.8)" : "rgba(15,25,28,0.76)");
      tower.addColorStop(1, "rgba(5,8,7,0.92)");
      ctx.fillStyle = tower;
      ...
      if ((place === "vegas" || place === "monaco") && i % 5 === 0) {
        ctx.fillStyle = `${design.light}66`;
        roundRect(x - bw * 0.25, farBase - bh - 12, bw * 1.5, 10, 3);
        ctx.fill();
      }
    }
  } else if (place === "coast" || place === "harbor" || place === "venice" || place === "rio") {
    ...
    if (place === "rio") {
      for (let i = 0; i < 7; i += 1) {
        const x = ((i * w * 0.17 - (raceState.roadOffset || 0) * 0.015) % (w + 180)) - 90;
        const ridgeH = h * (0.1 + seededUnit(seed, i + 18) * 0.16);
        ctx.fillStyle = "rgba(28,102,58,0.42)";
        ctx.beginPath();
        ctx.moveTo(x - w * 0.1, farBase + h * 0.04);
        ctx.quadraticCurveTo(x, farBase - ridgeH, x + w * 0.14, farBase + h * 0.04);
        ctx.closePath();
        ctx.fill();
      }
    }
    for (let i = 0; i < 9; i += 1) {
      ...
      ctx.fillStyle = place === "harbor" || place === "venice" ? "rgba(10,36,40,0.82)" : "rgba(10,31,34,0.76)";
      ctx.beginPath();
      ctx.ellipse(x, horizon + h * 0.085, w * 0.08, h * 0.028, 0, 0, Math.PI * 2);
      ctx.fill();
      if (place === "harbor" || place === "venice") {
  ...
  } else if (place === "alpine" || place === "snow" || place === "europe" || place === "iceland") {
    for (let i = 0; i < 6; i += 1) {
      const x = i * w * 0.2 - ((raceState.roadOffset || 0) * 0.01 % (w * 0.2));
      const peak = h * (0.18 + seededUnit(seed, i) * 0.13);
      ctx.fillStyle = place === "iceland" ? "rgba(150,185,190,0.46)" : "rgba(130,160,160,0.36)";
      ctx.beginPath();
...
function drawRoad(w, h, theme) {
  ...
  ctx.fill();
  ctx.save();
  ctx.globalAlpha = phoneMode ? 0.08 : 0.17;
  for (let i = 0; i < (phoneMode ? 44 : 130); i += 1) {
  ...
  const paintedLanes = phoneMode ? [-0.5, 0.5] : [-1.5, -0.5, 0.5, 1.5];
  paintedLanes.forEach((lane) => {
    for (let i = 0; i < (phoneMode ? 10 : 14); i += 1) {
      const y = ((i * 116 + raceState.roadOffset * 1.42) % (h + 190)) - 95;
      if (y < horizon) continue;
      const t = Math.max(0, (y - horizon) / (h - horizon));
      const dashH = 20 + t * (phoneMode ? 54 : 72);
      const x = baseRoadX(lane, t);
      const dash = ctx.createLinearGradient(x, y, x, y + dashH);
      dash.addColorStop(0, phoneMode ? "rgba(244,251,248,0.04)" : "rgba(244,251,248,0.08)");
      dash.addColorStop(0.35, phoneMode ? "rgba(244,251,248,0.44)" : "rgba(244,251,248,0.64)");
      dash.addColorStop(1, phoneMode ? "rgba(244,251,248,0.08)" : "rgba(244,251,248,0.12)");
      basePaintSegment(lane, y, dashH, 7 + t * (phoneMode ? 5 : 8), dash);
    }
  });
  if (!phoneMode) {
  ...
  ctx.strokeStyle = phoneMode ? "rgba(244,251,248,0.32)" : "rgba(244,251,248,0.58)";
  ctx.lineWidth = phoneMode ? 3 : 4;
  ctx.globalAlpha = phoneMode ? 0.26 : 0.7;
  ctx.beginPath();
...
function drawRoadsideDetails(w, h, theme, horizon, roadBottom) {
  const place = selectedRace && selectedRace.place ? selectedRace.place : "city";
  const phoneMode = phoneGraphicsActive();
  for (let i = 0; i < (phoneMode ? 9 : 22); i += 1) {
    ...
    if (place === "canyon") {
      ...
      if (phoneMode) {
        ctx.fillStyle = place === "coast" ? "rgba(38,118,76,0.44)" : "rgba(44,62,58,0.46)";
        ctx.beginPath();
        ctx.ellipse(x, y + size * 0.2, size * 1.05, size * 0.32, -0.08 * side, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = place === "coast" ? "rgba(70,217,255,0.18)" : "rgba(0,0,0,0.26)";
        ctx.beginPath();
        ctx.ellipse(x + side * size * 0.55, y + size * 0.4, size * 0.9, size * 0.18, -0.08 * side, 0, Math.PI * 2);
        ctx.fill();
        continue;
      }
      ctx.strokeStyle = "rgba(244,251,248,0.28)";
...
function drawObjects() {
  ...
  const canDrawPhoneVehicle = (gap, lane, priority = false) => {
    ...
    const readablePassWindow = priority && gap > -60 && gap < 420 && Math.abs(lane - raceState.lane) > 1.08;
    if (conflict && !readablePassWindow) return false;
...
function drawRealisticRearVehicleSignature(w, h, color, vehicleType = "car", police = false, air = false, water = false) {
  if (air || water) return;
  const heavy = ["semi", "truck", "monster", "tractor"].includes(vehicleType);
  const tank = vehicleType === "tank";
  const f1 = vehicleType === "f1" || vehicleType === "prototype";
  const led = police ? "#46d9ff" : heavy ? "#ff5b6b" : "#ff3348";
  ctx.save();
  ctx.globalAlpha = phoneGraphicsActive() ? 0.86 : 0.78;
  if (tank) {
    ctx.fillStyle = "rgba(4,6,4,0.68)";
    roundRect(-w * 0.34, h * 0.28, w * 0.68, h * 0.1, Math.max(3, w * 0.018));
    ctx.fill();
    ctx.fillStyle = "rgba(187,242,74,0.46)";
    for (let i = -3; i <= 3; i += 1) {
      ctx.beginPath();
      ctx.arc(i * w * 0.08, h * 0.33, Math.max(2, w * 0.014), 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    return;
  ...
  ctx.fillStyle = "rgba(3,5,5,0.88)";
  if (heavy) {
    roundRect(-w * 0.36, h * 0.18, w * 0.72, h * 0.16, Math.max(4, w * 0.026));
    ctx.fill();
  } else {
    ctx.beginPath();
  ...
  if (heavy) {
    roundRect(-w * 0.32, h * 0.19, w * 0.11, h * 0.055, 4);
    roundRect(w * 0.21, h * 0.19, w * 0.11, h * 0.055, 4);
    ctx.fill();
  } else {
    roundRect(-w * 0.32, h * 0.21, w * 0.2, h * 0.04, Math.max(3, w * 0.02));
    roundRect(w * 0.12, h * 0.21, w * 0.2, h * 0.04, Math.max(3, w * 0.02));
    ctx.fill();
    ctx.fillStyle = police ? "#ff3348" : "rgba(255,91,107,0.86)";
    roundRect(-w * 0.26, h * 0.27, w * 0.12, h * 0.018, 3);
    roundRect(w * 0.14, h * 0.27, w * 0.12, h * 0.018, 3);
    ctx.fill();
  }
  ctx.shadowBlur = 0;
  ctx.fillStyle = "rgba(244,251,248,0.76)";
  roundRect(-w * 0.08, h * (f1 ? 0.29 : 0.32), w * 0.16, h * 0.035, 4);
  ctx.fill();
  if (!heavy) {
    ctx.fillStyle = "rgba(1,2,2,0.96)";
    for (let i = -2; i <= 2; i += 1) {
      roundRect(i * w * 0.045 - w * 0.012, h * 0.39, w * 0.024, h * 0.09, 3);
      ctx.fill();
    }
    ctx.fillStyle = "rgba(0,0,0,0.86)";
    ctx.beginPath();
    ctx.ellipse(-w * 0.22, h * 0.43, w * 0.07, h * 0.025, 0, 0, Math.PI * 2);
    ctx.ellipse(w * 0.22, h * 0.43, w * 0.07, h * 0.025, 0, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.strokeStyle = "rgba(255,255,255,0.18)";
  ctx.lineWidth = Math.max(1, w * 0.01);
  ctx.beginPath();
  ctx.moveTo(-w * 0.36, -h * 0.04);
  ctx.bezierCurveTo(-w * 0.48, h * 0.12, -w * 0.43, h * 0.32, -w * 0.28, h * 0.46);
  ctx.moveTo(w * 0.36, -h * 0.04);
  ctx.bezierCurveTo(w * 0.48, h * 0.12, w * 0.43, h * 0.32, w * 0.28, h * 0.46);
  ctx.stroke();
  ctx.restore();
