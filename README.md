...
- Phone landscape mode has a separate in-game control dock inside the racing stage, so steering, gas, brake, boost, and the Hold/Toggle switch stay visible even when the side panel layout changes. The live race now sizes to the phone's actual visible viewport, uses smaller landscape controls on short screens, and leaves menu/garage scrolling enabled when a race is not live.
