...
- Phone graphics now add an extra cinematic realism pass with denser asphalt texture, tar cracks, rubber tire lanes, wet/oil reflections, stronger foreground depth, subtle film grain, route-tinted lighting, and higher-resolution vehicle paint/glass highlights. On phones, the app uses the clean mobile canvas renderer plus the lower hood camera so the first race stays readable and avoids stacked road layers; the camera buttons still let players switch back to full-car or windshield views.
